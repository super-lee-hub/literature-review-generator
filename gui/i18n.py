...
TRANSLATIONS["en"].update(
    {
        ...
        "高级：外部摘要文件（一般可跳过）": "Advanced: External Summary Files (Usually Optional)",
        "历史阶段一摘要会自动扫描复用。这里只用于跳过阶段一、直接用已有 summaries.json 生成大纲/正文，或补充 output_path 之外的复用池。":
            "Historical stage-1 summaries are scanned and reused automatically. Use this area only to skip stage 1 and generate an outline/review from an existing summaries.json file, or to add reuse pools outside output_path.",
...
EN_TO_ZH = {
    ...
    "Summary file": "摘要文件",
    "Use existing summaries.json for outline/review": "用已有 summaries.json 生成大纲/正文",
    ...
    "Additional outline/review summary sources (one path per line)": "额外的大纲/正文摘要来源（每行一个路径）",
    ...
    "Extra stage-1 reuse pools outside output_path (one path per line)": "output_path 之外的额外第一阶段复用池（每行一个路径）",
