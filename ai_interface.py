...
def _is_payload_parameter_error(parameter_name: str, *parts: Any) -> bool:
    text = " ".join(str(part or "") for part in parts).casefold()
    return parameter_name.casefold() in text and any(
        marker in text for marker in _PAYLOAD_PARAMETER_ERROR_MARKERS
    )


def _is_payload_value_error(
    parameter_name: str, parameter_value: str, *parts: Any
) -> bool:
    text = " ".join(str(part or "") for part in parts).casefold()
    return (
...
def _apply_optional_api_payload_params(
    payload: Dict[str, Any], api_config: APIConfig, logger: Any = None
) -> None:
    ...
    if thinking_payload:
        ...
        logger.warning(
            "Ignoring invalid API thinking config; expected enabled/disabled or {'type': ...}."
        )
...
def build_chat_completions_payload(
    ...
    *,
    ...
    normalized_user_content = _normalize_user_message_content(
        prompt, user_content, logger=logger
    )
    token_limit = _configured_positive_int(
        api_config.get("max_completion_tokens"), max_tokens
    )
...
def build_responses_payload(
    ...
    *,
    ...
    max_output_tokens = _configured_positive_int(
        api_config.get("max_output_tokens"), max_tokens
    )
    payload: Dict[str, Any] = {
        "model": api_config.get("model") or "",
        "input": [
            ...
            {
                "role": "user",
                "content": _convert_chat_content_to_responses_content(
...
def parse_responses_response(response_data: Dict[str, Any]) -> Tuple[str, str]:
    return _extract_response_output_text(response_data), _responses_finish_reason(
        response_data
    )
...
def _normalize_user_message_content(
    prompt: str, user_content: Any, logger: Any = None
) -> Any:
    ...
    for item in user_content:
        ...
        if item_type == "local_image_path":
            ...
            if not data_url:
                if logger:
                    logger.warning(
                        f"Skipping missing or unreadable local image input: {path}"
                    )
                continue
        ...
        if item_type == "image_url":
            ...
            if isinstance(image_url, dict) and image_url.get("url"):
                normalized.append(
                    {
                        "type": "image_url",
                        "image_url": {"url": str(image_url.get("url"))},
                    }
                )
        ...
        if item_type == "local_pdf_path":
            ...
            if not data_url:
                if logger:
                    logger.warning(
                        f"Skipping missing or unreadable local PDF input: {path}"
                    )
                continue
            normalized.append(
                {
...
def _call_ai_api_detailed(
    ...
    try:
        ...
        api_base = (
            api_config.get("api_base", "https://api.openai.com/v1")
        ...
        if not api_key or not model_name:
            ...
            if logger:
                logger.error(message)
            return _api_result(
                status="failed", error_kind="fatal_config_or_auth", message=message
            )
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                formatted = _format_success_result(
                    content, response_format, response, finish_reason, logger=logger
                )
                formatted.update(
                    {
                        "provider_response_id": str(response_data.get("id") or ""),
                        "response_model": str(response_data.get("model") or ""),
                        "usage": (
                            dict(response_data.get("usage") or {})
                ...
                if (
                    ...
                    and _is_payload_parameter_error(
                        "display", provider_code, message, last_failure["message"]
                    )
                    ...
                    if logger:
                        logger.warning(
                            "API rejected reasoning.display, retrying once without it."
                        )
                    continue
                ...
                if (
                    ...
                    and str(payload.get("reasoning_effort") or "").strip().casefold()
                    == "max"
                    and (
                        _is_payload_parameter_error(
                            "reasoning_effort",
                            provider_code,
                            message,
                            last_failure["message"],
                        )
                        or _is_payload_value_error(
                            "reasoning_effort",
                            "max",
                            provider_code,
                            message,
                    ...
                    if logger:
                        logger.warning(
                            "API rejected reasoning_effort=max, retrying once with high."
                        )
                    continue
                ...
                for payload_key in compat_payload_keys:
                    if (
                        ...
                        and _is_payload_parameter_error(
                            payload_key, provider_code, message, last_failure["message"]
                        )
                    ):
                ...
                if error_kind in {"quota_exhausted", "fatal_config_or_auth"}:
                    if logger:
                        logger.error(
                            f"API调用不可重试失败 ({error_kind}): {last_failure['message']}"
                        )
                    return last_failure
                ...
                if attempt < max_retries:
                    ...
                    if logger:
                        logger.warning(
                            f"{_response_error_details(response, limit=200)}，{wait_time:.1f}秒后重试..."
                        )
                    time.sleep(wait_time)
                    continue
                ...
                if isinstance(response_status, int) and response_status >= 400:
                    error_kind, http_status, provider_code, message = (
                ...
                if attempt < max_retries:
                    ...
                    if logger:
                        logger.warning(
                            f"API调用失败 ({error_kind}): {message}，{wait_time:.1f}秒后重试..."
                        )
                    time.sleep(wait_time)
                    continue
...
def _call_ai_api_text_detailed(
    ...
    """Call the configured API for text content and retain the full receipt.

    The underlying detailed transport already normalizes chat-completions and
    Responses API metadata. Returning that mapping unchanged keeps provider
    model, response id, usage, retry counts, and failure classification
    available to downstream provenance records.
    """
...
def _aggressive_json_fix(json_str: str) -> str:
    ...
    # 变量类型注解
    result: str = (
        '{"error": "无法修复JSON格式", "original_content": '
        + json.dumps(json_str[:200])
        + "}"
    )
    return result
...
class RateLimiter:
    ...
    def __init__(
        self,
        primary_tpm_limit: int = 900000,
        primary_rpm_limit: int = 9000,
        backup_tpm_limit: int = 2000000,
        backup_rpm_limit: int = 9000,
    ) -> None:
        """
        初始化双引擎令牌桶限速器
    ...
    def _refill_primary(self):
        ...
        if time_passed > 0:
            ...
            self.primary_tpm_tokens = min(
                self.primary_tpm_capacity, self.primary_tpm_tokens + new_tokens
            )
            self.primary_tpm_last_refill = current_time

            # 补充主引擎RPM令牌
            new_requests = time_passed * self.primary_rpm_refill_rate
            self.primary_rpm_tokens = min(
                self.primary_rpm_capacity, self.primary_rpm_tokens + new_requests
            )
    ...
    def _refill_backup(self):
        ...
        if time_passed > 0:
            ...
            self.backup_tpm_tokens = min(
                self.backup_tpm_capacity, self.backup_tpm_tokens + new_tokens
            )
            self.backup_tpm_last_refill = current_time

            # 补充备用引擎RPM令牌
            new_requests = time_passed * self.backup_rpm_refill_rate
            self.backup_rpm_tokens = min(
                self.backup_rpm_capacity, self.backup_rpm_tokens + new_requests
            )
            self.backup_rpm_last_refill = current_time

    def consume(
        self, tokens_needed: int, requests_needed: int = 1, engine_type: str = "primary"
        ...
        if engine_type not in ["primary", "backup"]:
            raise ValueError(
                f"未知的引擎类型: {engine_type}，必须是 'primary' 或 'backup'"
            )
        ...
        with self.lock:
            if engine_type == "primary":
                ...
                if (
                    ...
                    and self.primary_rpm_tokens >= requests_needed
                ):
                    # 原子性消耗主引擎令牌
                    self.primary_tpm_tokens -= tokens_needed
                    ...
                    self._log(
                        "debug",
                        f"主引擎主动模式消耗成功 - TPM: {tokens_needed}/{int(self.primary_tpm_tokens)}, RPM: {requests_needed}/{int(self.primary_rpm_tokens)}",
                    )
                    return True
                else:
                    ...
                    if (
                        self.primary_tpm_refill_rate > 0
                        and self.primary_tpm_tokens < tokens_needed
                    ):
                        tpm_wait = (
                            tokens_needed - self.primary_tpm_tokens
                        ) / self.primary_tpm_refill_rate

                    if (
                        self.primary_rpm_refill_rate > 0
                        and self.primary_rpm_tokens < requests_needed
                    ):
                        rpm_wait = (
                            requests_needed - self.primary_rpm_tokens
                        ) / self.primary_rpm_refill_rate

                    wait_time = max(tpm_wait, rpm_wait)

                    self._log(
                        "info", f"主引擎主动模式令牌不足，需要等待: {wait_time:.2f}秒"
                    )
                    return wait_time
                ...
                if (
                    ...
                    and self.backup_rpm_tokens >= requests_needed
                ):
                    # 原子性消耗备用引擎令牌
                    self.backup_tpm_tokens -= tokens_needed
                    ...
                    self._log(
                        "debug",
                        f"备用引擎主动模式消耗成功 - TPM: {tokens_needed}/{int(self.backup_tpm_tokens)}, RPM: {requests_needed}/{int(self.backup_rpm_tokens)}",
                    )
                    return True
                else:
                    # 计算需要等待的时间（使用更安全的方式避免除零错误）
                    ...
                    if (
                        self.backup_tpm_refill_rate > 0
                        and self.backup_tpm_tokens < tokens_needed
                    ):
                        tpm_wait = (
                            tokens_needed - self.backup_tpm_tokens
                        ) / self.backup_tpm_refill_rate

                    if (
                        self.backup_rpm_refill_rate > 0
                        and self.backup_rpm_tokens < requests_needed
                    ):
                        rpm_wait = (
                            requests_needed - self.backup_rpm_tokens
                        ) / self.backup_rpm_refill_rate

                    wait_time = max(tpm_wait, rpm_wait)

                    self._log(
                        "info", f"备用引擎主动模式令牌不足，需要等待: {wait_time:.2f}秒"
                    )
                    return wait_time
            else:
        ...
        with self.lock:
            ...
            if engine_type == "primary":
                return {
                    ...
                    "tpm_usage_percent": (
                        self.primary_tpm_capacity - self.primary_tpm_tokens
                    )
                    / self.primary_tpm_capacity
                    * 100,
                    "rpm_tokens": self.primary_rpm_tokens,
                    "rpm_capacity": self.primary_rpm_capacity,
                    "rpm_usage_percent": (
                        self.primary_rpm_capacity - self.primary_rpm_tokens
                    )
                    / self.primary_rpm_capacity
                    * 100,
                }
            elif engine_type == "backup":
                return {
                    ...
                    "tpm_usage_percent": (
                        self.backup_tpm_capacity - self.backup_tpm_tokens
                    )
                    / self.backup_tpm_capacity
                    * 100,
                    "rpm_tokens": self.backup_rpm_tokens,
                    "rpm_capacity": self.backup_rpm_capacity,
                    "rpm_usage_percent": (
                        self.backup_rpm_capacity - self.backup_rpm_tokens
                    )
                    / self.backup_rpm_capacity
                    * 100,
                }
            else:  # 'all'
                return {
                    "primary_tpm_usage_percent": (
                        self.primary_tpm_capacity - self.primary_tpm_tokens
                    )
                    / self.primary_tpm_capacity
                    * 100,
                    "backup_tpm_usage_percent": (
                        self.backup_tpm_capacity - self.backup_tpm_tokens
                    )
                    / self.backup_tpm_capacity
                    * 100,
                    "primary_rpm_usage_percent": (
                        self.primary_rpm_capacity - self.primary_rpm_tokens
                    )
                    / self.primary_rpm_capacity
                    * 100,
                    "backup_rpm_usage_percent": (
                        self.backup_rpm_capacity - self.backup_rpm_tokens
                    )
                    / self.backup_rpm_capacity
                    * 100,
...
# 引擎映射表，统一引擎名称和日志术语
engine_map = {
    "primary": {"name": "主阅读引擎", "short_name": "主引擎"},
    "backup": {"name": "备用阅读引擎", "short_name": "备用引擎"},
}
...
try:
    ...
    # 支持0值，表示被动模式
    _primary_tpm_limit = safe_int_convert(
        _config.get("Performance", {}).get("primary_tpm_limit", 900000), 900000
    )
    _primary_rpm_limit = safe_int_convert(
        _config.get("Performance", {}).get("primary_rpm_limit", 9000), 9000
    )
    _backup_tpm_limit = safe_int_convert(
        _config.get("Performance", {}).get("backup_tpm_limit", 2000000), 2000000
    )
    _backup_rpm_limit = safe_int_convert(
        _config.get("Performance", {}).get("backup_rpm_limit", 9000), 9000
    )
    rate_limiter = RateLimiter(
        _primary_tpm_limit, _primary_rpm_limit, _backup_tpm_limit, _backup_rpm_limit
    )
...
def get_summary_from_ai_detailed(
    ...
    if "dummy" in (primary_api_config.get("api_key") or "") or "dummy" in (
        backup_api_config.get("api_key") or ""
    ):
        return _api_result(
            status="success",
            engine_type=engine_type,
    ...
    if system_prompt is None:
        try:
            with open(
                "prompts/prompt_system_analyze.txt",
                "r",
                encoding="utf-8",
            ) as handle:
            ...
            system_prompt = (
                "You are an academic literature analysis expert. Analyze the paper "
                "text and return a structured JSON summary."
            )
    elif not system_prompt.strip():
        return _api_result(
            status="failed",
            error_kind="fatal_config_or_auth",
            message="System prompt text is empty",
            engine_type=engine_type,
        )
...
def get_summary_from_ai_with_fallback(
    prompt_text: str,
    primary_api_config: APIConfig,
    backup_api_config: APIConfig,
    logger: Optional[Any] = None,
    ...
    while True:
        ...
        for engine in engine_order:
            ...
            if logger:
                logger.info(
                    f"Stage 1 reader attempt: {engine}#{attempt_budget - remaining[engine]}"
                )
...
def get_summary_from_ai(
    ...
    if engine_type in engine_map:
        api_config = (
            primary_api_config if engine_type == "primary" else backup_api_config
        )
        engine_name = engine_map[engine_type]["name"]
    else:
        raise ValueError(f"未知的引擎类型: {engine_type}")
    ...
    try:
        if config:
            if engine_type == "primary":
                max_tokens = int(
                    config.get("API_Parameters", {}).get("primary_max_tokens", 3000)
                )
                temperature = float(
                    config.get("API_Parameters", {}).get("primary_temperature", 0.3)
                )
            else:  # backup
                max_tokens = int(
                    config.get("API_Parameters", {}).get("backup_max_tokens", 8192)
                )
                temperature = float(
                    config.get("API_Parameters", {}).get("backup_temperature", 0.3)
                )
        else:
            # 默认值（向后兼容）
            max_tokens = 3000
    ...
    # 调用令牌桶控制器
    rate_limit_result = rate_limiter.consume(
        tokens_needed=estimated_tokens, requests_needed=1, engine_type=engine_type
    )
    ...
    if isinstance(ai_response, dict):  # type: ignore
        ...
        if not isinstance(structured_summary.get("common_core"), dict):
            if logger:
                logger.error(
                    f"common_core类型错误: {type(structured_summary.get('common_core'))}"
                )
        ...
        for field in required_fields:
            if field not in structured_summary["common_core"]:
                structured_summary["common_core"][field] = (
                    "" if field != "key_points" else []
                )
        ...
        if not isinstance(structured_summary["common_core"]["key_points"], list):
            structured_summary["common_core"]["key_points"] = [
                str(structured_summary["common_core"]["key_points"])
            ]

        # 确保type_specific_details存在
        if "type_specific_details" not in structured_summary:
            structured_summary["type_specific_details"] = (
                _default_type_specific_details()
            )
        else:
            structured_summary["type_specific_details"] = (
                _normalize_type_specific_details(
                    structured_summary["type_specific_details"]
                )
            )
...
def _extract_summary_manually(
    ...
    for match in json_matches:
        try:
            ...
            if isinstance(
                json_data, dict
            ):  # json.loads可能返回任何JSON类型，需要检查是否为字典
                # 提取common_core部分
    ...
    for field, field_patterns in patterns.items():
        for pattern in field_patterns:
            matches: List[str] = re.findall(
                pattern, ai_response_str, re.IGNORECASE | re.DOTALL
            )
            if matches:
                if field == "key_points":
