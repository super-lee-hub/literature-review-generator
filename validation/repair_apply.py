...
def check_apply_guards(
    ...
    4. Optional auto-safe guard - only for structural auto-safe policy paths
    """
    ...
    auto_safe_guard_passed = True
    if require_auto_safe:
        auto_safe_guard_passed = is_auto_safe_proposal(proposal)

    if require_auto_safe and not auto_safe_guard_passed:
        block_reasons.append("Proposal is not eligible for auto_safe structural apply")

    can_apply = (
        version_guard_passed
        and anchor_hash_guard_passed
        and dependency_guard_passed
        and (not require_auto_safe or auto_safe_guard_passed)
    )
    ...
    return ApplyGuardResult(
        ...
        auto_safe_guard_passed=auto_safe_guard_passed,
    )
...
def _set_manifest_repair_annotation(
    citation_manifest: Dict[str, Any],
    proposal: PatchProposal,
    job_id: str,
) -> bool:
    """Annotate citation manifest entries without changing review text."""
    annotation = {
        "proposal_id": proposal.proposal_id,
        "plan_id": proposal.metadata.get("plan_id", ""),
        "citation_id": proposal.citation_id,
        "root_cause": proposal.root_cause.value,
    ...
    target_ids = {
        str(value).strip()
        for value in [
            proposal.citation_id,
            proposal.metadata.get("validation_bundle_id"),
            proposal.metadata.get("citation_set_key"),
            proposal.metadata.get("claim_unit_id"),
        ]
        if str(value or "").strip()
    }
    matched = False

    for key in ("citation_sets", "occurrences", "citations"):
        entries = citation_manifest.get(key, [])
        ...
        for entry in entries:
            ...
            entry_ids = {
                ...
                for value in [
                    entry.get("bundle_id"),
                    entry.get("citation_set_key"),
                    entry.get("occurrence_id"),
                    entry.get("citation_id"),
                ]
                if str(value or "").strip()
            }
            claim_unit_ids = {
                str(unit.get("claim_unit_id")).strip()
                for unit in entry.get("claim_units", [])
                if isinstance(unit, dict) and str(unit.get("claim_unit_id") or "").strip()
            }
            if target_ids and not (target_ids & (entry_ids | claim_unit_ids)):
                continue
            entry.setdefault("repair_annotations", []).append(dict(annotation))
            matched = True

    if not matched:
        citation_manifest.setdefault("repair_annotations", []).append(dict(annotation))
        matched = True
    return matched


def apply_structural_patch(
    proposal: PatchProposal,
    review_draft: Dict[str, Any],
    citation_manifest: Dict[str, Any],
    paper_artifacts: Sequence[Dict[str, Any]],
    ...
    """Apply an auto-safe structural patch without mutating review text."""
    ...
    original_text = _get_block_text(review_draft, proposal.target.block_id)
    if original_text is None:
        return None

    if not _set_manifest_repair_annotation(citation_manifest, proposal, job_id):
        return None

    anchor_hash = _compute_anchor_hash(original_text)
    return AppliedPatchRecord(
        ...
        anchor_hash_before=anchor_hash,
        anchor_hash_after=anchor_hash,
    )
...
class RepairApplier:
    ...
    def __init__(
        ...
        self.require_auto_safe = require_auto_safe
    ...
    def apply_all(self) -> RepairApplyResult:
        ...
        for proposal in self.repair_plan.proposals:
            ...
            guard_result = check_apply_guards(
                ...
                expected_artifact_version=expected_version,
                require_auto_safe=self.require_auto_safe,
            )
            ...
            # Apply patch
            if self.require_auto_safe:
                record = apply_structural_patch(
                    proposal,
                    self.review_draft,
                    self.citation_manifest,
    ...
    def get_patched_citation_manifest(self) -> Dict[str, Any]:
        """Get the citation manifest after structural repair annotations."""
        return self.citation_manifest
...
def run_repair_apply(
    ...
    applier = RepairApplier(
        ...
        require_auto_safe=require_auto_safe,
    )
    ...
    patched_manifest = applier.get_patched_citation_manifest()
