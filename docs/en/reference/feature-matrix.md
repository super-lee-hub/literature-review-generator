...
The current offline baseline is 730 collected tests, 708 passed, and 22
...
| Stage plan and `run_all` policy | E2E_VERIFIED | durable stage plan requests analyze/outline/review/validate when validation is enabled, omits only optional validation when disabled, still requires `CurrentArtifactSet`, and blocks derivation/outline-only canonical readiness without it |
