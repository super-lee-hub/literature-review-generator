...
当前离线基线为 collected 748、passed 726、deselected 22。Live API、
...
| Stage 1 与证据 | E2E_VERIFIED | 不可变 content-addressed summaries、paper artifact、evidence manifest、edge checkpoint；expected-call closure 显式记录 reuse，reuse 时 provider call 为零且不合成 receipt；summary -> source_bundle -> 来源 PDF 依赖链 |
...
| Stage plan 与 `run_all` policy | E2E_VERIFIED | validation 启用时持久化 plan 固定为 analyze/outline/review/validate；禁用时只省略 optional validation 并发布 typed not-requested disposition，但仍要求 `CurrentArtifactSet`；派生或 outline-only 不能在没有 current set 时 canonical-ready |
| Outline quality 与 stability gates | E2E_VERIFIED | typed `OutlineQualityGate`、effective-section/duplicate/placeholder/empty-stream audit、一个额外的完整 reversed-summary smoke chain、full-decision stability variants、逐节点 call/token/cost plan、显式 pricing source、zero-transport exact replay 和 gate-hash invalidation |
...
| Queue 状态映射 | E2E_VERIFIED | Queue 读取 `job_status`；旧 success 仅为 readiness 投影；lease-generation staging 按 queue-lock -> Registry 顺序发布 immutable bytes、写入 lease publication manifest，并覆盖 Windows `spawn` stale-worker/orphan |
