...
from docx_writer import (
    ...
    generate_word_table_of_contents,
    generate_apa_references_from_manifest,
)
from report_generator import (
    generate_excel_report,
    generate_failure_report,
    generate_retry_zotero_report,
)  # type: ignore
...
from context_manager import (
    ...
    optimize_context_for_synthesis,
    optimize_context_for_outline,
    estimate_tokens,
)
...
if not DOCX_AVAILABLE:
    logging.warning(
        "未安装 'python-docx'。生成Word文档和第二阶段验证功能将不可用。请运行: pip install python-docx"
    )
...
class CheckpointManager:
    ...
    def save_checkpoint(self, generator: "LiteratureReviewGenerator") -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"[断点保存] 已保存处理进度: {len(processed_papers)}成功, {len(failed_papers)}失败"
            )
            return True
    ...
    def load_checkpoint(self, generator: "LiteratureReviewGenerator") -> bool:
        ...
        try:
            ...
            if version != "2.0":
                self.logger.warning(
                    f"[断点加载] 检测到旧版本断点文件(v{version})，将开始全新处理"
                )
                return False
            ...
            if checkpoint_project != generator.project_name:
                self.logger.warning(
                    f"[断点加载] 项目名称不匹配({checkpoint_project} != {generator.project_name})，将开始全新处理"
                )
                return False
            ...
            processing_stats: Dict[str, Any] = (
                checkpoint_data.get("processing_stats") or {}
            )
...
class LiteratureReviewGenerator:
    ...
    FREE_MODE_PROFILE_ARTIFACT_ID = "free_mode_profile:active"
    FREE_MODE_PROFILE_ARTIFACT_TYPE = "free_mode_profile"
    WRITER_PROMPT_PROVENANCE_ARTIFACT_ID = "writer_prompt_context_provenance:v1"
    WRITER_PROMPT_PROVENANCE_ARTIFACT_TYPE = "writer_prompt_context_provenance"
    ...
    def bind_job_workspace(
        ...
        *,
        ...
        self.summary_file = workspace.artifact_path(
            f"{workspace.project_name}_summaries.json"
        )
    ...
    def _attach_workspace_log_handler(
        self, workspace: JobWorkspace, artifact_registry: ArtifactRegistry
    ) -> None:
        ...
        if (
            ...
            and self._workspace_log_handler in logger_handlers
        ):
            return
    ...
    def retry_queue_job(self, job_id: str) -> bool:
        ...
        return (
            ...
            and self.queue_service.increment_retry_count(job_id) > 0
        )
    ...
    def _get_stage1_checkpoint_file_path(self) -> str:
        if self.job_workspace and self.project_name:
            return self.job_workspace.checkpoint_path(
                f"{self.project_name}_checkpoint.json"
            )
    ...
    def _get_summary_file_path(self) -> str:
        ...
        if self.job_workspace and self.project_name:
            return self.job_workspace.artifact_path(
                f"{self.project_name}_summaries.json"
            )
        if self.output_dir and self.project_name:
    ...
    def _get_summary_source_manifest_path(self) -> str:
        if self.job_workspace and self.project_name:
            return self.job_workspace.artifact_path(
                f"{self.project_name}_summary_source_manifest.json"
            )
        if self.output_dir and self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_summary_source_manifest.json"
            )
    ...
    def _get_summary_reuse_report_path(self) -> str:
        if self.job_workspace and self.project_name:
            return self.job_workspace.artifact_path(
                f"{self.project_name}_summary_reuse_report.json"
            )
        if self.output_dir and self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_summary_reuse_report.json"
            )
    ...
    def _get_concept_profile_file_path(self) -> str:
        if self.job_workspace and self.project_name:
            return self.job_workspace.artifact_path(
                f"{self.project_name}_concept_profile.json"
            )
        if self.output_dir and self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_concept_profile.json"
            )
    ...
    def _legacy_citation_policy(self) -> str:
        if self.compat_config:
            legacy_policy_getter = getattr(
                self.compat_config, "legacy_citation_policy", None
            )
            if callable(legacy_policy_getter):
                return (
                    str(legacy_policy_getter() or "report_only").strip()
                    or "report_only"
                )
        ...
        if isinstance(validation, Mapping):
            return (
                str(validation.get("legacy_citation_policy") or "report_only").strip()
                or "report_only"
            )
        return "report_only"
    ...
    def _format_citation_ref_catalog_for_prompt(
        ...
        for summary in self.summaries:
            paper_info_raw = (
                summary.get("paper_info", {}) if isinstance(summary, Mapping) else {}
            )
        ...
        for entry in catalog.get("entries", []):
            if not isinstance(entry, Mapping) or entry.get("status") != "active":
                continue
            evidence_summary = ""
            summary = summaries_by_key.get(
                str(entry.get("canonical_paper_key") or "").strip()
            ) or summaries_by_key.get(str(entry.get("paper_id") or "").strip())
    ...
    def _outline_v2_model_call(
        self, route_name: str, prompt: str, metadata: Dict[str, Any]
    ) -> Any:
        """Call the configured v2 model route and return provider JSON."""
        config = self.config or {}
        api_params = (
            config.get("API_Parameters", {})
            if isinstance(config.get("API_Parameters", {}), dict)
            else {}
        )
        ...
        try:
            max_tokens = int(
                api_config.get("max_tokens")
                or api_params.get(max_token_key)
        ...
        try:
            temperature = float(
                api_params.get(
                    max_token_key.replace("max_tokens", "temperature"),
                    default_temperature,
                )
            )
        except (TypeError, ValueError):
        ...
        try:
            timeout_seconds = int(
                api_params.get(timeout_key)
                or api_params.get("outline_v2_timeout_seconds")
        ...
        try:
            retry_attempts = int(
                api_params.get(retries_key)
                or api_params.get("outline_v2_retry_attempts")
                or (2 if stage == "outline_candidates" else 1)
            )
        except (TypeError, ValueError):
        ...
        metadata.update(
            {
                "provider_route": route_name,
                "configured_model": str(api_config.get("model") or ""),
        ...
        metadata.update(
            {
                ...
                .isoformat()
                + "Z",
                "transport_status": str(response.get("status") or "failed"),
                "response_model": str(response.get("response_model") or ""),
                "provider_response_id": str(response.get("provider_response_id") or ""),
                "attempt_count": int(response.get("attempt_count") or 0),
                "http_attempt_count": int(response.get("http_attempt_count") or 0),
                "http_status": response.get("http_status"),
                "error_kind": str(response.get("error_kind") or ""),
                "provider_message": str(response.get("message") or ""),
            }
        )
        if response.get("status") != "success":
            message = str(
                response.get("message") or response.get("error_kind") or "unknown error"
            )
            self.logger.error(
                f"Outline v2 provider request failed: stage={stage}, "
                f"candidate_index={candidate_index}, message={message}"
            )
            raise RuntimeError(
                f"Outline v2 provider request failed for {stage}: {message}"
            )
        content = str(response.get("content") or "")
        parsed = _smart_json_parser(content) or _auto_correct_json(content)
        if parsed is not None:
            self.logger.info(
                f"Outline v2 model call parsed JSON: stage={stage}, candidate_index={candidate_index}"
            )
            return parsed
        self.logger.error(
        ...
        metadata["transport_status"] = "failed"
        metadata["error_kind"] = metadata.get("error_kind") or "invalid_response"
        raise ValueError(f"Outline v2 provider returned unparseable JSON for {stage}")

    def _load_paper_artifacts_for_outline_v2(self) -> List[Dict[str, Any]]:
        if not self.job_workspace:
            return []
        artifacts_dir = os.path.join(
            self.job_workspace.paths.artifacts_dir, "paper_artifacts"
        )
    ...
    def _load_literature_map_for_citation_resolution(self) -> Optional[Dict[str, Any]]:
        ...
        if not self.job_workspace or not self.project_name:
            return None
        path = self.job_workspace.artifact_path(
            f"{self.project_name}_literature_map.json"
        )
        ...
        try:
            ...
            self.logger.warning(
                f"读取 literature_map 以解析引用别名失败，已跳过 {path}: {exc}"
            )
            return None
    ...
    def adopt_outline(self) -> bool:
        ...
        try:
            if self._outline_v2_enabled():
                self.logger.error(
                    "当前启用了 Outline Intelligence v2；请使用 --adopt-outline-v2。"
                )
                return False
            ...
            outline_doc_path = self.job_workspace.artifact_path(
                f"{self.project_name}_outline_document.json"
            )
            ...
            # 加载仲裁结果（如果存在）
            arbitration_path = self.job_workspace.artifact_path(
                f"{self.project_name}_outline_arbitration.json"
            )
            ...
            # 保存采纳后的大纲（使用正确的文件名）
            reviewed_outline_path = self.job_workspace.artifact_path(
                f"{self.project_name}_reviewed_outline.json"
            )
            ...
            if self.artifact_registry:
                ...
                if os.path.exists(outline_doc_path):
                    depends_on.append(
                        ArtifactDependencyRef(
                            artifact_type="outline_document", path=outline_doc_path
                        )
                    )
                if os.path.exists(arbitration_path):
                    depends_on.append(
                        ArtifactDependencyRef(
                            artifact_type="outline_arbitration", path=arbitration_path
                        )
                    )
    ...
    def adopt_outline_v2(
        ...
        try:
            ...
            final_path = self.job_workspace.artifact_path(
                f"{self.project_name}_final_outline.json"
            )
            audit_path = self.job_workspace.artifact_path(
                f"{self.project_name}_outline_coverage_audit.json"
            )
            ...
            if (
                ...
                self.logger.error(
                    "Registered outline_stage_health evidence is missing or stale"
                )
                return False
            ...
            adopted, message = adopt_final_outline(
                ...
                allow_arbitration_fallback=str(
                    (self.config or {})
                    .get("OutlineQualityGate", {})
                    .get("allow_arbitration_fallback_adoption", "false")
                )
                .strip()
                .lower()
                in {"1", "true", "yes", "on"},
            ...
            adopted_path = self.job_workspace.artifact_path(
                f"{self.project_name}_adopted_final_outline.json"
            )
    ...
    def _write_stage1_progress_snapshot(self) -> bool:
        ...
        snapshot_path = self.job_workspace.artifact_path(
            "stage1_progress_snapshot.json"
        )
    ...
    def _persist_paper_artifact(self, result: Mapping[str, Any]) -> bool:
        ...
        try:
            ...
            evidence_manifest = build_evidence_manifest_v1(
                ...
                preprocess=paper_artifact_payload.get("analysis", {}).get(
                    "preprocess", {}
                ),
            )
            ...
            paper_artifact_payload.setdefault("stage1_inputs", {})[
                "evidence_manifest_path"
            ] = evidence_manifest_path
            paper_artifact_payload["stage1_inputs"]["evidence_manifest_hash"] = (
                file_sha256(evidence_manifest_path)
            )
            ...
            source_pdf = str(
                paper_artifact_payload.get("source", {}).get("source_pdf") or ""
            )
            if source_pdf:
                ...
                expected_source_hash = str(
                    paper_artifact_payload.get("source", {}).get(
                ...
                if (
                    ...
                    and source_dependency.content_hash != expected_source_hash
                ):
                    raise ValueError(
                        "source PDF hash changed before paper artifact registration"
                    )
                depends_on.append(source_dependency)
            visual_manifest_path = str(
                paper_artifact_payload.get("stage1_inputs", {}).get(
                    "visual_artifact_manifest_path"
                )
                or ""
            )
    ...
    def _review_draft_path(self) -> str:
        ...
        return self.job_workspace.artifact_path(
            f"review_drafts/{project_name}_review_draft_v1.json"
        )
    ...
    def _review_draft_v2_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"review_drafts/{project_name}_review_draft_v2.json"
        )
    ...
    def _citation_manifest_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"citation_manifests/{project_name}_citation_manifest_v3.json"
        )
    ...
    def _citation_ref_catalog_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"citation_catalogs/{project_name}_citation_ref_catalog.json"
        )
    ...
    def _citation_manifest_v2_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"citation_manifests/{project_name}_citation_manifest_v2.json"
        )
    ...
    def _citation_manifest_v1_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"citation_manifests/{project_name}_citation_manifest_v1.json"
        )
    ...
    def _citation_migration_report_path(self) -> str:
        ...
        project_name = self.project_name or "review"
        return self.job_workspace.artifact_path(
            f"citation_manifests/{project_name}_citation_migration_report.json"
        )
        ...
        *,
        ...
        try:
            summaries = (
                ...
                else [dict(summary) for summary in self.summaries]
            )
            ...
            profile_dependency = self._register_free_mode_profile_input()
            if profile_dependency is not None:
                depends_on.append(profile_dependency)
            writer_provenance_dependency = self._writer_prompt_provenance_dependency()
            if writer_provenance_dependency is not None:
                depends_on.append(writer_provenance_dependency)
        ...
        try:
            ...
            def _flush_current_section() -> None:
                ...
                section_map[current_section_number] = {
                    ...
                    "section_title": section_titles_by_number.get(
            ...
            self.logger.warning(
                f"Failed to extract existing review sections from Word document: {exc}"
            )
            return {}
        ...
        *,
        ...
        try:
            ...
            citation_ref_catalog_hash = str(
                (citation_ref_catalog or {}).get("catalog_hash") or ""
            )
    ...
    def _persist_citation_manifest(
        ...
        *,
        ...
        try:
            ...
            atomic_write_json(
                self._citation_migration_report_path(),
                citation_manifest_v3.migration_report.to_dict(),
            )
    ...
    def _init_logger(self):
        ...
        self.logger = logging.getLogger(
            f"auto-generate_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )  # type: ignore
    ...
    def setup_output_directory(self) -> bool:
        ...
        try:
            ...
            paths_config: Dict[str, str] = (
                self.config.get("Paths", {}) if self.config else {}
            )
            ...
            if self.job_workspace is None:
                ...
                if os.path.exists(pointer_path):
                    try:
                        ...
                        workspace_path = str(
                            pointer_payload.get("workspace_path", "") or ""
                        )
                        ...
                        self.logger.warning(
                            f"读取 latest job pointer 失败，将创建新 workspace: {exc}"
                        )
                if self.job_workspace is None:
            ...
            for i, pdf_path in enumerate(pdf_files):
                ...
                if "_" in title:
                    ...
                    if len(parts) >= 2:
                        ...
                        if (
                            potential_author
                            and potential_author != "侯甜甜"
                            and potential_author != "贺爱忠"
                            and potential_author != "周冲"
                            and potential_author != "盘城"
                            and potential_author != "张赛楠"
                            and potential_author != "彭丽徽"
                            and potential_author != "康超"
                            and potential_author != "刘伟华"
                            and potential_author != "朱华东"
                        ):
        ...
        try:
            ...
            for diagnostic in parse_result.diagnostics:
                if diagnostic.severity == "error":
                    self.logger.error(
                        f"Zotero parser [{diagnostic.code}]: {diagnostic.message}"
                    )
    ...
    def _materialize_effective_summaries(
        ...
        *,
        ...
        source_dependencies_override: Optional[Sequence[ArtifactDependencyRef]] = None,
    ...
    @staticmethod
    def _project_legacy_summary_to_canonical(
        ...
        authors = paper_info.get("authors")
        has_authors = (
            bool(authors.strip()) if isinstance(authors, str) else bool(authors)
        )
        ...
        *,
        ...
        for index, source in enumerate(sources):
            ...
            rejection_reasons = sorted(
                rejection_reasons_by_path.get(normalized_path, set())
            )
        ...
        if self.artifact_registry and self.job_workspace:
            for source in sources:
                ...
                if os.path.normcase(resolved_path) not in contributing_paths:
                    continue
                path_key = hashlib.sha256(resolved_path.encode("utf-8")).hexdigest()[
                    :12
                ]
                source_record = self.artifact_registry.register_file(
        ...
        self.logger.success(
            f"Loaded summary sources: {success_count} success, 0 failed"
        )
        if resolved.rejected_candidates:
            self.logger.info(
                f"Summary source merge skipped {len(resolved.rejected_candidates)} non-reusable records"
            )
        ...
        if (
            ...
            if manifest_record is None:
                raise RuntimeError(
                    "materialized summary source manifest was not registered"
                )
            input_refs = [
                AuditArtifactRefV1(
                    artifact_id=record.artifact_id,
            ...
            reason = (
                self.audit_reason.strip() or "explicit legacy summary source selected"
            )
            ...
            audit_path = self.job_workspace.artifact_path(
                f"legacy_reuse_{audit.audit_id}.json"
            )
    ...
    def _load_summaries_from_path(
        ...
        *,
        ...
        self.summaries = [
            cast(ProcessingResult, dict(item)) for item in loaded_summaries
        ]
        ...
        self.logger.success(
            f"Loaded summary file: {success_count} success, {failed_count} failed"
        )
        ...
        try:
            ...
            if explicit_summary_sources:
                source_kind = (
                    ...
                    if len(explicit_summary_sources) == 1
                    else "explicit_summary_sources"
                )
            ...
            if self.summary_file and os.path.exists(self.summary_file):
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Current summary file is invalid; starting fresh instead: {exc}"
                    )
                    self.summaries = []
            ...
            if self.project_name:
                ...
                workspace_pattern = os.path.join(
                    output_base_path_abs, f"{self.project_name}__*"
                )
                ...
                for workspace_path in workspaces:
                    ...
                    try:
                        self.logger.info(
                            f"Found historical summary file: {summary_file}"
                        )
                        return self._load_summaries_from_path(summary_file)
                    except SummarySourceError as exc:
                        self.logger.warning(
                            f"Historical summary file is invalid; checking the next candidate: {exc}"
                        )
                        continue
            ...
            if explicit_summary_sources:
                ...
                return False
            self.logger.warning(
                f"Failed to load existing summary file; starting a fresh run: {e}"
            )
            self.summaries = []
            return True
    ...
    def _restore_stage1_progress_from_snapshot(self, snapshot_path: str) -> bool:
        ...
        try:
            ...
            processed_papers = {
                ...
                and normalize_checkpoint_paper_key(paper_key)
            }
            failed_papers = {
                normalize_checkpoint_paper_key(paper_key)
            ...
            self.logger.warning(
                f"[进度恢复] 读取 progress snapshot 失败，将继续尝试其他恢复路径: {exc}"
            )
            return False
    ...
    def _rebuild_stage1_progress_from_loaded_summaries(self) -> bool:
        ...
        processed_papers, failed_papers = (
            self._stage1_progress_sets_from_loaded_summaries()
        )
    ...
    def _disable_stage1_reader_engine_for_round(
        self, engine_type: str, result: Mapping[str, Any]
    ) -> None:
        label = "主引擎" if engine_type == "primary" else "备用引擎"
        message = f"{label}余额/额度不足，本轮自动跳过。"
        reason = str(
            result.get("message")
            or result.get("provider_code")
    ...
    def _call_stage1_reader_with_scheduler(
        ...
        *,
        ...
        if result:
            legacy_engine = (
                "backup" if skip_engines and "primary" in skip_engines else "primary"
            )
            return {
                "status": "success",
                "error_kind": None,
    ...
    def _stage1_strategy_policy(self) -> str:
        preprocess_section = (
            (self.config or {}).get("Preprocess", {}) if self.config else {}
        )
        policy = (
            str(preprocess_section.get("strategy_policy") or "auto").strip().lower()
        )
        allowed = {
            "auto",
            "formal_precision",
    ...
    def _collect_preprocess_result_metadata(
        self, preprocess_result: Any, input_kind: str
    ) -> Dict[str, Any]:
        metadata = {
            ...
            "mineru_attempted": bool(
                getattr(preprocess_result, "mineru_attempted", False)
            ),
            "mineru_succeeded": bool(
                getattr(preprocess_result, "mineru_succeeded", False)
            ),
            "mineru_token_present": bool(
                getattr(preprocess_result, "mineru_token_present", False)
            ),
            "mineru_remote_requested": bool(
                getattr(preprocess_result, "mineru_remote_requested", False)
            ),
            "mineru_remote_enabled": bool(
                getattr(preprocess_result, "mineru_remote_enabled", False)
            ),
            ...
            "structured_json_path": getattr(
                preprocess_result, "structured_json_path", ""
            ),
            ...
            "stage1_input_manifest_path": getattr(
                preprocess_result, "stage1_input_manifest_path", ""
            ),
            "stage1_quality_report_path": getattr(
                preprocess_result, "stage1_quality_report_path", ""
            ),
            "selected_text_source": getattr(
                preprocess_result, "selected_text_source", ""
            ),
            "stage1_quality_level": getattr(
        ...
        if quality_report_path:
            try:
                ...
                metadata["stage1_quality_reasons"] = quality_report.get(
                    "stage1_quality_reasons", []
                )
                metadata["stage1_completeness_metrics"] = quality_report.get(
        ...
        if manifest_path:
            try:
                ...
                metadata["selected_text_length"] = int(
                    stage1_manifest.get("selected_text_length") or 0
                )
                metadata["stage1_page_count"] = int(
                    stage1_manifest.get("page_count") or 0
                )
                if not metadata.get("stage1_completeness_metrics"):
                    metadata["stage1_completeness_metrics"] = stage1_manifest.get(
                        "completeness_metrics", {}
                    )
            except Exception:
    ...
    def _fresh_preprocess_manager_for_strategy(
        self, preprocess_strategy: str
    ) -> PreprocessManager:
    ...
    @staticmethod
    def _stage1_route_snapshot(
        strategy: str, metadata: Mapping[str, Any]
    ) -> Dict[str, Any]:
        ...
        requested = (
            bool(metadata.get("mineru_remote_requested"))
            if "mineru_remote_requested" in metadata
            else remote_candidate
        )
        enabled = (
            bool(metadata.get("mineru_remote_enabled"))
            if "mineru_remote_enabled" in metadata
            else (strategy == "mineru" or parser_mode in {"remote", "remote_first"})
        )
        ...
        return {
            ...
            "allow_local_parse_fallback": bool(
                metadata.get("allow_local_parse_fallback")
            ),
            "extractor_used": text("extractor_used", "unknown"),
            "selected_text_source": text("selected_text_source")
            or text("analysis_input_kind"),
            "stage1_quality_level": text("stage1_quality_level"),
            "stage1_quality_reasons": list(
                metadata.get("stage1_quality_reasons") or []
            ),
            ...
            "primary_text_source": text("primary_text_source")
            or text("selected_text_source")
            or text("analysis_input_kind"),
            ...
            "pdf_attachment_size_mb": float(
                metadata.get("pdf_attachment_size_mb") or 0
            ),
            "text_only_evidence_used": text("text_only_evidence_used"),
            "fallback_reason": text("fallback_reason")
            or text("stage1_input_fallback_reason"),
            "quality_report_path": text("quality_report_path")
            or text("stage1_quality_report_path"),
    ...
    def _stage1_route_human_message(snapshot: Mapping[str, Any]) -> str:
        ...
        if route == "hybrid_local_baseline_met":
            return (
                "配置把 MinerU 作为候选，但本地基线达标，所以本篇未发起 MinerU 请求。"
            )
    ...
    def _load_stage1_prompt_template(self) -> str:
        """加载阶段一结构化分析提示词模板。"""
        with open(
            "prompts/optimized_prompt_analyze_router.txt", "r", encoding="utf-8"
        ) as handle:
            return handle.read()

    def _prepare_stage1_input(
        self, pdf_path: str, preprocess_strategy: str = "hybrid"
            ...
            preprocess_metadata.update(
                {
                    ...
                    "stage1_quality_reasons": [
                        "MINERU_API_TOKEN is required for mineru strategy"
                    ],
                }
            )
        ...
        if self.preprocess_manager and preprocess_strategy != "legacy":
            strategy_manager = self._fresh_preprocess_manager_for_strategy(
                preprocess_strategy
            )
            try:
                if preprocess_strategy == "mineru":
                    self.logger.info(
                        f"强制使用 MinerU 远程解析策略: {os.path.basename(pdf_path)}"
                    )
                elif preprocess_strategy == "docling":
                    self.logger.info(
                        f"强制使用 Docling 解析策略: {os.path.basename(pdf_path)}"
                    )
                ...
                if preprocess_result:
                    ...
                    input_kind = (
                        preprocess_result.selected_text_source or "stage1_input"
                    )
                    preprocess_metadata.update(
                        self._collect_preprocess_result_metadata(
                            preprocess_result, input_kind
                        )
                    )
                    ...
                    if is_blocked_stage1_quality(
                        ...
                        preprocess_metadata["analysis_input_kind"] = (
                            preprocess_result.selected_text_source
                            or "blocked_stage1_input"
                        )
                        self.logger.warning(
        ...
        if preprocess_strategy == "legacy":
            self.logger.info(
                f"阶段一输入使用旧版 PDF 文本提取: {os.path.basename(pdf_path)}"
            )
            legacy_text = str(extract_text_from_pdf(pdf_path) or "")  # type: ignore
            mineru_base_url = (
                str(os.getenv("MINERU_BASE_URL", "https://mineru.net/api/v4"))
                .strip()
                .rstrip("/")
            )
            preprocess_metadata.update(
    ...
    def _extract_doi_from_text(value: Any) -> str:
        ...
        url_match = re.search(
            r"https?://(?:dx\.)?doi\.org/(10\.\d{4,9}/\S+)", text, flags=re.IGNORECASE
        )
        if url_match:
            text = url_match.group(1)
        else:
            doi_match = re.search(
                r"(10\.\d{4,9}/[-._;()/:A-Z0-9]+)", text, flags=re.IGNORECASE
            )
            if doi_match:
    ...
    def _is_metadata_noise_line(line: str) -> bool:
        ...
        if re.search(r"\b(?:vol|issue|no\.)\b", lowered) and re.search(
    ...
    def _split_author_candidates(cls, value: Any) -> List[str]:
        ...
        if (
            not text
            or cls._is_metadata_noise_line(text)
            or cls._is_affiliation_line(text)
        ):
            return []
    ...
    def _extract_citation_metadata(
        cls, lines: List[str], title_hint: str
    ) -> Dict[str, Any]:
        citation_line = next(
            (line for line in lines[:80] if "to cite this article" in line.casefold()),
            "",
        )
        ...
        if doi:
            rest = re.sub(
                rf",?\s*(?:DOI|doi)\s*:?\s*{re.escape(doi)}.*$",
                "",
                rest,
                flags=re.IGNORECASE,
            )
        ...
        segments = [
            cls._normalize_metadata_scan_text(segment) for segment in rest.split(",")
        ]
    ...
    @classmethod
    def _extract_stage1_metadata(
        cls, stage1_text: str, paper: Mapping[str, Any]
    ) -> Dict[str, Any]:
        if not stage1_text:
            return {}
        ...
        lines = [
            cls._normalize_metadata_scan_text(line) for line in scan_text.splitlines()
        ]
        ...
        if title_hint_key:
            for index, line in enumerate(lines[:80]):
                ...
                if not line_key:
                    continue
                if (
                    line_key == title_hint_key
                    or title_hint_key in line_key
                    or line_key in title_hint_key
                ):
                    title_index = index
                    break
        ...
        if title_index is not None:
            ...
            if not metadata.get("journal"):
                for index in range(title_index - 1, max(-1, title_index - 6), -1):
                    ...
                    if cls._is_metadata_noise_line(
                        candidate
                    ) or cls._is_affiliation_line(candidate):
                        continue
                    if re.search(r"\b(?:19|20)\d{2}\b", candidate):
                        continue
    ...
    def _apply_stage1_text_metadata_backfill(
        self, paper: PaperInfo, stage1_text: str
    ) -> List[str]:
        needs_backfill = (
        ...
        metadata = sanitize_metadata_fields(
            self._extract_stage1_metadata(stage1_text, paper)
        )
    ...
    def _resolve_free_mode_context(self) -> str:
        ...
        if profile is None and profile_path:
            try:
                ...
                self.logger.warning(
                    f"加载自由模式 profile 失败，将回退到临时意图文本: {exc}"
                )
    ...
    def _inject_free_mode_context(self, prompt_template: str) -> str:
        ...
        prompt_with_placeholder = prompt_template.replace(
            "{{FREE_MODE_CONTEXT}}", free_mode_context
        )
        if free_mode_context and free_mode_context not in prompt_with_placeholder:
            return f"{free_mode_context}\n{prompt_with_placeholder}"
        return prompt_with_placeholder

    def _register_free_mode_profile_input(self) -> Optional[ArtifactDependencyRef]:
        raw_profile_path = str(
            getattr(self, "free_mode_profile_path", None) or ""
        ).strip()
        if not raw_profile_path:
            return None
        profile_path = os.path.abspath(raw_profile_path)
        if str(getattr(self, "free_mode_idea", None) or "").strip():
            raise ValueError(
                "free_mode_profile and free_mode_idea are mutually exclusive"
            )
        if not os.path.isfile(profile_path):
            raise FileNotFoundError(f"free-mode profile is missing: {profile_path}")

        profile_hash = file_sha256(profile_path)
        if self.artifact_registry is None or self.job_workspace is None:
            return ArtifactDependencyRef(
                artifact_type=self.FREE_MODE_PROFILE_ARTIFACT_TYPE,
                path=profile_path,
                content_hash=profile_hash,
            )

        current = self.artifact_registry.get(self.FREE_MODE_PROFILE_ARTIFACT_ID)
        if not (
            current is not None
            and current.status == "ready"
            and os.path.abspath(current.path) == profile_path
            and current.content_hash == profile_hash
        ):
            context_hash = hashlib.sha256(
                self._resolve_free_mode_context().encode("utf-8")
            ).hexdigest()
            current = self.artifact_registry.register_file(
                artifact_role="prompt_profile_input",
                artifact_type=self.FREE_MODE_PROFILE_ARTIFACT_TYPE,
                artifact_version="v1",
                path=profile_path,
                producer="main.LiteratureReviewGenerator._register_free_mode_profile_input",
                artifact_id=self.FREE_MODE_PROFILE_ARTIFACT_ID,
                metadata={"prompt_context_sha256": context_hash},
            )
    ...
    def _writer_prompt_provenance_path(self) -> str:
        if not self.job_workspace:
            return ""
        return self.job_workspace.artifact_path(
            f"{self.project_name or 'review'}_writer_prompt_context_provenance_v1.json"
        )

    def _writer_prompt_provenance_dependency(self) -> Optional[ArtifactDependencyRef]:
        if self.artifact_registry is None:
            return None
        record = self.artifact_registry.get(self.WRITER_PROMPT_PROVENANCE_ARTIFACT_ID)
        if (
            record is None
            or record.status != "ready"
            or not os.path.isfile(record.path)
        ):
            return None
        return ArtifactDependencyRef(
            artifact_type=record.artifact_type,
    ...
    def _persist_writer_prompt_context_provenance(
        self,
        *,
        ...
        provenance_path = self._writer_prompt_provenance_path()
        if not provenance_path or self.artifact_registry is None:
            return

        profile_dependency = self._register_free_mode_profile_input()
        prompt_context = self._resolve_free_mode_context()
        context_hash = hashlib.sha256(prompt_context.encode("utf-8")).hexdigest()
        context_occurrences = (
            effective_prompt.count(prompt_context) if prompt_context else 0
        )
        context_present = bool(prompt_context and context_occurrences == 1)
        if prompt_context and not context_present:
            raise ValueError(
                "Writer effective prompt does not contain exactly one profile context"
            )
        ...
        if os.path.isfile(provenance_path):
            ...
            if not isinstance(loaded, dict):
                raise ValueError(
                    "Writer prompt-context provenance must be a JSON object"
                )
            existing = loaded
            prior_hash = str(existing.get("expected_prompt_context_sha256") or "")
            if prior_hash and prior_hash != context_hash:
                raise ValueError("Writer prompt context changed within the same job")

        requests = list(existing.get("requests") or [])
        request_entry = {
            "request_index": len(requests) + 1,
            "writer_stage": writer_stage,
            "provider_route": "Writer_API",
            "provider_family": str(api_config.get("provider_family") or ""),
            "configured_model": str(api_config.get("model") or ""),
            ...
            "profile_path": os.path.abspath(str(self.free_mode_profile_path or "")),
            "profile_file_sha256": (
                profile_dependency.content_hash
                if profile_dependency is not None
                else ""
            ),
            "prompt_context_present": context_present,
            "prompt_context_occurrences": context_occurrences,
        ...
        all_context_ok = bool(requests) and all(
            bool(item.get("prompt_context_present"))
            and str(item.get("prompt_context_sha256") or "") == context_hash
            and int(item.get("prompt_context_occurrences") or 0) == 1
            for item in requests
            if isinstance(item, Mapping)
        )
        payload = {
            "schema_version": "writer-prompt-context-provenance-v1",
            "job_id": self.job_workspace.job_id if self.job_workspace else "",
            "project_name": self.project_name or "review",
            "profile_path": os.path.abspath(str(self.free_mode_profile_path or "")),
            "profile_file_sha256": (
                profile_dependency.content_hash
            ...
            "expected_prompt_context_sha256": context_hash,
            "all_requests_use_expected_context": all_context_ok,
            "request_count": len(requests),
            "requests": requests,
        }
        atomic_write_json(provenance_path, payload)
        dependencies = [profile_dependency] if profile_dependency is not None else []
        self.artifact_registry.register_file(
            artifact_role="writer_prompt_context_provenance",
            artifact_type=self.WRITER_PROMPT_PROVENANCE_ARTIFACT_TYPE,
            artifact_version="v1",
            path=provenance_path,
            producer="main.LiteratureReviewGenerator._persist_writer_prompt_context_provenance",
            artifact_id=self.WRITER_PROMPT_PROVENANCE_ARTIFACT_ID,
    ...
    def _call_writer_api_text_detailed(
        self,
        *,
        prompt: str,
        api_config: APIConfig,
        ...
        from outline.prompt_budget import PromptBudgetV1, estimate_prompt_tokens

        self._register_free_mode_profile_input()
        effective_prompt = self._inject_free_mode_context(prompt)
        prompt_context = self._resolve_free_mode_context()
        if prompt_context and effective_prompt.count(prompt_context) != 1:
            raise ValueError(
                "Writer effective prompt must contain exactly one profile context"
            )

        context_limit = int(api_config.get("max_context_tokens") or 200000)
        budget = PromptBudgetV1(
            model_context_limit=context_limit,
            max_output_tokens=int(max_tokens),
            system_prompt_tokens=estimate_prompt_tokens(system_prompt),
        )
        budget.assert_fits(effective_prompt, stage=writer_stage)
        budget_metadata = budget.metadata(effective_prompt)
        ...
        self._persist_writer_prompt_context_provenance(
            effective_prompt=effective_prompt,
            prompt_budget=budget_metadata,
            api_config=api_config,
            result=result,
            writer_stage=writer_stage,
    ...
    def _build_stage1_visual_bundle(
        ...
        *,
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to build stage-1 visual bundle, continuing with text-first fallback: {exc}"
            )
            return None
    ...
    def _build_stage1_model_input(
        ...
        *,
        ...
        if paper and self.artifact_registry and self.job_workspace:
            ...
            paper_key = self._paper_artifact_key(paper)
            artifact_hash = self._paper_artifact_hash(paper_key)
            paper_artifact_path = self.job_workspace.artifact_path(
                f"paper_artifacts/{artifact_hash}.json"
            )

            # Resolve selected visual refs from paper artifact or registry
            selected_visual_refs = resolver.resolve_selected_visual_refs(
                paper_artifact_path
            )
            if selected_visual_refs:
                ...
                manifest_path = resolver.resolve_visual_manifest_path(
                    paper_artifact_path
                )
                if manifest_path:
    ...
    def _get_outline_file_path(self) -> str:
        ...
        if self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_literature_review_outline.md"
            )
    ...
    def _get_legacy_outline_file_path(self) -> str:
        if self.project_name:
            ...
            if self.config:
                output_base_path = self.config.get("Paths", {}).get(
                    "output_path", "./output"
    ...
    def _resolve_outline_file_path(self) -> Optional[str]:
        if self.artifact_registry:
            ...
            if record and not os.path.exists(record.path):
                self.logger.warning(
                    f"Registered outline artifact is missing on disk: {record.path}"
                )

        workspace_outline_file = self._get_outline_file_path()
        if os.path.exists(workspace_outline_file):
            self.logger.info(
                f"Using workspace outline file without registry lookup fallback: {workspace_outline_file}"
            )
            return workspace_outline_file

        legacy_outline_file = self._get_legacy_outline_file_path()
        if legacy_outline_file != workspace_outline_file and os.path.exists(
            legacy_outline_file
        ):
            self.logger.warning(
                f"Using legacy outline compatibility fallback: {legacy_outline_file}"
            )
    ...
    def _load_outline_artifact(self) -> Optional[Tuple[str, str]]:
        ...
        if self._outline_v2_enabled():
            ...
            resolver = OutlineRuntimeResolver(
                config=self.config or {},
                artifact_registry=self.artifact_registry,
                workspace_path=self.job_workspace.paths.root_dir
                if self.job_workspace
                else (self.output_dir or ""),
                project_name=self.project_name or "",
            ...
            if resolved is None:
                ...
                return None
            self.logger.info(
                f"Using v2 adopted outline artifact: {resolved.source_path}"
            )
        ...
        self.logger.error(
            "No outline artifact was found in the current workspace/registry or legacy fallback path"
        )
        return None
    ...
    def _get_review_checkpoint_file_path(self) -> str:
        ...
        if self.job_workspace is not None and self.project_name:
            return self.job_workspace.checkpoint_path(
                f"{self.project_name}_review_checkpoint.json"
            )
        if self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_review_checkpoint.json"
            )
    ...
    def _get_review_word_file_path(self) -> str:
        if not self.output_dir:
            raise ValueError("输出目录未设置")
        if self.job_workspace is not None and self.project_name:
            return self.job_workspace.report_path(
                f"{self.project_name}_literature_review.docx"
            )
        if self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_literature_review.docx"
            )
    ...
    def _get_failed_review_sections_file_path(self) -> str:
        ...
        if self.job_workspace is not None and self.project_name:
            return self.job_workspace.report_path(
                f"{self.project_name}_failed_review_sections.json"
            )
        if self.project_name:
            return os.path.join(
                self.output_dir, f"{self.project_name}_failed_review_sections.json"
            )
    ...
    def _is_placeholder_metadata_value(value: Any) -> bool:
        ...
        return any(
            candidate.casefold() in placeholder_keywords
            or candidate in placeholder_texts
    ...
    def _is_metadata_only_quality_failure(self, reason: str) -> bool:
        ...
        return bool(parts) and all(
            self._is_metadata_quality_issue(part) for part in parts
        )
    ...
    def _apply_paper_metadata_to_ai_summary(
        self, paper: PaperInfo, ai_summary: Any
    ) -> List[str]:
        ...
        def set_if_missing(field: str, value: Any) -> None:
            ...
            current_missing = (
                not current
                or (
                    field in {"year", "journal"}
                    and self._is_placeholder_metadata_value(current)
                )
    ...
    def _resolve_stage1_metadata_for_quality(
        self, paper: PaperInfo, ai_summary: Any, stage1_text: str
    ) -> List[str]:
        updated_fields: List[str] = []
        try:
        ...
        try:
            updated_fields.extend(
                self._apply_stage1_text_metadata_backfill(paper, stage1_text)
            )
        except Exception as exc:
            self.logger.warning(f"Stage-1 text metadata backfill failed: {exc}")
        try:
            updated_fields.extend(
                self._apply_paper_metadata_to_ai_summary(paper, ai_summary)
            )
        except Exception as exc:
            self.logger.warning(f"Summary metadata sync failed: {exc}")
        return updated_fields

    def _missing_metadata_fields_for_summary(
        self, paper: PaperInfo, ai_summary: Any
    ) -> List[str]:
    ...
    def _mark_summary_metadata_manual_review(
        self, paper: PaperInfo, ai_summary: Any, quality_reason: str
    ) -> List[str]:
        if not isinstance(ai_summary, dict):
            return []
        ...
        if (
            ...
            and "metadata_needs_manual_review" not in conflict_flags
        ):
            conflict_flags.append("metadata_needs_manual_review")
        return missing_fields

    def _mark_metadata_manual_review_if_missing(
        self, paper: PaperInfo, ai_summary: Any, reason: str
    ) -> List[str]:
        missing_fields = self._missing_metadata_fields_for_summary(paper, ai_summary)
    ...
    def _apply_ai_metadata_backfill(
        self, paper: PaperInfo, ai_summary: Any
    ) -> List[str]:
        metadata = sanitize_metadata_fields(get_paper_metadata(ai_summary or {}))
        updated_fields: List[str] = []
    ...
    def _save_failed_review_sections(
        self, failed_sections: List[Dict[str, Any]]
    ) -> None:
    ...
    def _load_failed_review_sections(self) -> List[Dict[str, Any]]:
        ...
        try:
            with open(failed_sections_file, "r", encoding="utf-8") as handle:
                payload = json.load(handle)
            failed_sections = (
                payload.get("failed_sections", []) if isinstance(payload, dict) else []
            )
    ...
    def _trim_review_document_from_section(
        self, word_file: str, section_number: int
    ) -> bool:
        if not os.path.exists(word_file):
            return True
        if not DOCX_AVAILABLE or Document is None:
            self.logger.warning(
                "python-docx 不可用，将退回整篇重生而不是裁剪已有文档。"
            )
            return False
    ...
    def generate_specific_review_section(self, section_number: int) -> bool:
        try:
            ...
            if outline_artifact is None:
                self.logger.error(
                    f"大纲文件不存在: {outline_file}，请先运行 --generate-outline"
                )
                return False

            _outline_file, outline_content = outline_artifact

            section_title = self.extract_section_title_from_outline(
                outline_content, section_number
            )
            if not section_title:
                self.logger.error(f"未在大纲中找到第 {section_number} 章")
                return False

            return self.create_literature_review_section(
                section_number, section_title, outline_content
            )
        except Exception as exc:
    ...
    def retry_failed_review_sections(self) -> bool:
        try:
            ...
            if os.path.exists(word_file) and self._trim_review_document_from_section(
                word_file, first_failed_section
            ):
    ...
    def _persist_source_identity_result(
        ...
        artifact_suffix = hashlib.sha256(stable_identity.encode("utf-8")).hexdigest()[
            :20
        ]
    ...
    def process_paper(
        ...
        try:
            ...
            if paper_key in self._checkpoint_processed_papers:
                ...
                for summary in self.summaries:
                    if (
                        ...
                        and LiteratureReviewGenerator.get_paper_key(
                            summary.get("paper_info", {})
                        )
                        == paper_key
                    ):
                        return summary
            ...
            if paper_key in self._checkpoint_failed_papers:
                self.logger.info(
                    f"重新尝试上次失败论文: {paper.get('title', '未知标题')}"
                )
                self._checkpoint_failed_papers.discard(paper_key)

            self.logger.info(
                f"[{paper_index + 1}/{total_papers}] 正在处理: {paper.get('title', '未知标题')}"
            )
            ...
            self._emit_progress(
                stage="analyze",
                item_label=paper_label,
                message=f"正在处理: {paper_label}",
            )
            ...
            if not pdf_path and self.mode == "zotero":
                ...
                library_path: str = self.library_path or paths_config.get(
                    "library_path", ""
                )
                ...
                if find_result:
                    ...
                    self.logger.error(
                        f"未找到PDF文件: {file_title} - 原因: {failure_reason}"
                    )
                    return {
                        "paper_info": paper,
                        "status": "failed",
            ...
            def record_attempt_failure(
                ...
                *,
                ...
                route_snapshot = self._stage1_route_snapshot(
                    strategy_name, current_metadata
                )
                attempt_history.append(
                    {
                        "preprocess_strategy": strategy_name,
                        "preprocess_profile": str(
                            route_snapshot.get("preprocess_profile") or strategy_name
                        ),
                        "parser_mode": str(route_snapshot.get("parser_mode") or ""),
                        "extractor_used": extractor_name
                        or str(route_snapshot.get("extractor_used") or "unknown"),
                        "selected_text_source": str(
                            route_snapshot.get("selected_text_source") or ""
                        ),
                        "stage1_quality_level": str(
                            route_snapshot.get("stage1_quality_level") or ""
                        ),
                        "stage1_quality_reasons": list(
                            route_snapshot.get("stage1_quality_reasons") or []
                        ),
                        "selected_text_length": int(
                            route_snapshot.get("selected_text_length") or 0
                        ),
                        "stage1_page_count": int(
                            route_snapshot.get("stage1_page_count") or 0
                        ),
                        "strategy_policy": str(
                            route_snapshot.get("strategy_policy") or strategy_policy
                        ),
                        "selected_strategy": str(
                            route_snapshot.get("selected_strategy") or strategy_name
                        ),
                        "input_mode": str(route_snapshot.get("input_mode") or ""),
                        "formal_input_path": str(
                            route_snapshot.get("formal_input_path") or ""
                        ),
                        "primary_text_source": str(
                            route_snapshot.get("primary_text_source") or ""
                        ),
                        "pdf_file_input_supported": bool(
                            route_snapshot.get("pdf_file_input_supported")
                        ),
                        "pdf_attachment_status": str(
                            route_snapshot.get("pdf_attachment_status") or ""
                        ),
                        "original_pdf_attached": bool(
                            route_snapshot.get("original_pdf_attached")
                        ),
                        "pdf_attachment_reason": str(
                            route_snapshot.get("pdf_attachment_reason") or ""
                        ),
                        "pdf_attachment_size_mb": float(
                            route_snapshot.get("pdf_attachment_size_mb") or 0
                        ),
                        "text_only_evidence_used": str(
                            route_snapshot.get("text_only_evidence_used") or ""
                        ),
                        "fallback_reason": str(
                            route_snapshot.get("fallback_reason") or ""
                        ),
                        "quality_report_path": str(
                            route_snapshot.get("quality_report_path") or ""
                        ),
                        "mineru_remote_requested": bool(
                            route_snapshot.get("mineru_remote_requested")
                        ),
                        "mineru_remote_enabled": bool(
                            route_snapshot.get("mineru_remote_enabled")
                        ),
                        "mineru_attempted": bool(
                            route_snapshot.get("mineru_attempted")
                        ),
                        "mineru_succeeded": bool(
                            route_snapshot.get("mineru_succeeded")
                        ),
            ...
            for strategy in preprocess_strategies:
                ...
                # 准备阶段一输入
                self.logger.info(
                    f"正在准备阶段一输入 (策略: {strategy}): {os.path.basename(pdf_path)}"
                )
                self._emit_progress(
                    stage="analyze",
                    item_label=paper_label,
                    message=f"正在准备阶段一输入 (策略: {strategy}): {os.path.basename(pdf_path)}",
                )

                pdf_text, preprocess_metadata = self._prepare_stage1_input(
                    pdf_path, strategy
                )
                stage1_route_snapshot = self._stage1_route_snapshot(
                    strategy, preprocess_metadata
                )
                preprocess_metadata["stage1_route"] = stage1_route_snapshot
                self.logger.info(
                    self._format_stage1_route_snapshot(stage1_route_snapshot)
                )
                route_human_message = self._stage1_route_human_message(
                    stage1_route_snapshot
                )
                ...
                if not pdf_text or len(pdf_text.strip()) < 500:  # type: ignore
                    ...
                    stage1_reasons = [
                        str(reason)
                        for reason in (
                            preprocess_metadata.get("stage1_quality_reasons") or []
                        )
                        if str(reason).strip()
                    ]
                    if stage1_reasons:
                ...
                if self.mode == "zotero":
                    ...
                    if not identity_result.canonical_ready:
                        failure_reason = (
                            f"source_identity_{identity_result.identity_verdict}"
                        )
                        self.logger.error(
                ...
                try:
                    updated_fields = self._apply_stage1_text_metadata_backfill(
                        paper, pdf_text
                    )
                    if updated_fields:
                        self.logger.info(
                            f"已从阶段一输入回填元数据字段: {', '.join(updated_fields)}"
                        )
                except Exception as e:
                ...
                self._emit_progress(
                    stage="analyze",
                    item_label=paper_label,
                    message=f"正在调用AI生成摘要: {paper_label}",
                )
                ...
                preprocess_metadata["visual_artifact_manifest_path"] = str(
                    stage1_input.get("visual_manifest_path") or ""
                )
                preprocess_metadata["visual_bundle_path"] = str(
                    stage1_input.get("visual_bundle_path") or ""
                )
                preprocess_metadata["selected_visual_count"] = len(
                    stage1_input.get("selected_visual_refs") or []
                )
                preprocess_metadata["stage1_input_mode"] = str(
                    stage1_input.get("input_mode") or "text_only"
                )
                preprocess_metadata["stage1_input_fallback_reason"] = str(
                    stage1_input.get("fallback_reason") or ""
                )
                for snapshot_key in (
                ...
                preprocess_metadata["input_mode"] = str(
                    stage1_input.get("input_mode") or "text_only"
                )
                preprocess_metadata["fallback_reason"] = str(
                    stage1_input.get("fallback_reason") or ""
                )
                preprocess_metadata["primary_text_source"] = str(
                    preprocess_metadata.get("selected_text_source") or input_kind
                )
                pdf_required = strategy_policy in {
                    "formal_precision",
                    "mineru_first",
                    "mineru_only",
                } and self._truthy_config(
                    self._stage1_input_settings().get(
                ...
                if not ai_result:
                    api_message = str(
                        reader_result.get("message")
                        or reader_result.get("error_kind")
                        or ""
                    )
                ...
                try:
                    ...
                    if updated_fields:
                        self.logger.info(
                            f"已从 AI 结果回填元数据字段: {', '.join(updated_fields)}"
                        )
                    else:
                        self.logger.info(
                            "未检测到可回填的论文元数据，继续保留现有 paper_info。"
                        )
                except Exception as e:
                    self.logger.warning(f"元数据回填失败: {e}")

                try:
                    synced_fields = self._apply_paper_metadata_to_ai_summary(
                        paper, ai_result
                    )
                    if synced_fields:
                        self.logger.info(
                            f"Synced paper_info metadata into AI summary: {', '.join(synced_fields)}"
                        )
                except Exception as e:
                ...
                if not is_quality_ok:
                    if self._is_metadata_only_quality_failure(quality_reason):
                        resolved_fields = self._resolve_stage1_metadata_for_quality(
                            paper, ai_result, pdf_text
                        )
                        if resolved_fields:
                            self.logger.info(
                                f"Metadata-only quality issue resolved fields: {', '.join(resolved_fields)}"
                            )
                        temp_result = {
                            "paper_info": paper,
                            "status": "success",
                        ...
                        is_quality_ok, quality_reason = validate_summary_quality(
                            temp_result
                        )
                        if is_quality_ok:
                    ...
                    self.logger.warning(
                        f"策略 {strategy} 主引擎质量检查失败: {failure_reason}"
                    )
                    ...
                    if backup_api_key and backup_api_key.strip():
                        ...
                        backup_model_used = str(
                            backup_reader_result.get("engine_type") or "backup"
                        )
                        ...
                        if backup_result:
                            ...
                            # 检查备用引擎结果的质量
                            try:
                                updated_fields = self._apply_ai_metadata_backfill(
                                    paper, backup_result
                                )
                                if updated_fields:
                                    self.logger.info(
                                        f"已从备用 AI 结果回填元数据字段: {', '.join(updated_fields)}"
                                    )
                            except Exception as e:
                            ...
                            is_quality_ok_backup, quality_reason_backup = (
                                validate_summary_quality(temp_result_backup)
                            )

                            if (
                                not is_quality_ok_backup
                                and self._is_metadata_only_quality_failure(
                                    quality_reason_backup
                                )
                            ):
                                resolved_fields = (
                                    self._resolve_stage1_metadata_for_quality(
                                        paper,
                                        backup_result,
                                        pdf_text,
                                    )
                                ...
                                if self._is_metadata_only_quality_failure(
                                    quality_reason_backup
                                ):
                                    missing_fields = (
                                        self._mark_summary_metadata_manual_review(
                                            paper,
                                            backup_result,
                                            quality_reason_backup,
                                        )
                            ...
                            if is_quality_ok_backup:
                                ...
                                self.logger.warning(
                                    f"备用引擎内容质量检查也失败: {quality_reason_backup}"
                                )
            ...
            attempt_history.append(
                {
                    "preprocess_strategy": str(
                        final_stage1_route_snapshot.get("strategy") or strategy
                    ),
                    "preprocess_profile": str(
                        final_stage1_route_snapshot.get("preprocess_profile")
                        or strategy
                    ),
                    "parser_mode": str(
                        final_stage1_route_snapshot.get("parser_mode") or ""
                    ),
                    "extractor_used": str(
                        final_stage1_route_snapshot.get("extractor_used") or "unknown"
                    ),
                    "selected_text_source": str(
                        final_stage1_route_snapshot.get("selected_text_source") or ""
                    ),
                    "stage1_quality_level": str(
                        final_stage1_route_snapshot.get("stage1_quality_level") or ""
                    ),
                    "stage1_quality_reasons": list(
                        final_stage1_route_snapshot.get("stage1_quality_reasons") or []
                    ),
                    "selected_text_length": int(
                        final_stage1_route_snapshot.get("selected_text_length")
                        or len(pdf_text or "")
                    ),
                    "stage1_page_count": int(
                        final_stage1_route_snapshot.get("stage1_page_count") or 0
                    ),
                    "strategy_policy": str(
                        final_stage1_route_snapshot.get("strategy_policy")
                        or strategy_policy
                    ),
                    "selected_strategy": str(
                        final_stage1_route_snapshot.get("selected_strategy") or strategy
                    ),
                    "input_mode": str(
                        final_stage1_route_snapshot.get("input_mode") or ""
                    ),
                    "formal_input_path": str(
                        final_stage1_route_snapshot.get("formal_input_path") or ""
                    ),
                    "primary_text_source": str(
                        final_stage1_route_snapshot.get("primary_text_source") or ""
                    ),
                    "pdf_file_input_supported": bool(
                        final_stage1_route_snapshot.get("pdf_file_input_supported")
                    ),
                    "pdf_attachment_status": str(
                        final_stage1_route_snapshot.get("pdf_attachment_status") or ""
                    ),
                    "original_pdf_attached": bool(
                        final_stage1_route_snapshot.get("original_pdf_attached")
                    ),
                    "pdf_attachment_reason": str(
                        final_stage1_route_snapshot.get("pdf_attachment_reason") or ""
                    ),
                    "pdf_attachment_size_mb": float(
                        final_stage1_route_snapshot.get("pdf_attachment_size_mb") or 0
                    ),
                    "text_only_evidence_used": str(
                        final_stage1_route_snapshot.get("text_only_evidence_used") or ""
                    ),
                    "fallback_reason": str(
                        final_stage1_route_snapshot.get("fallback_reason") or ""
                    ),
                    "quality_report_path": str(
                        final_stage1_route_snapshot.get("quality_report_path") or ""
                    ),
                    "mineru_remote_requested": bool(
                        final_stage1_route_snapshot.get("mineru_remote_requested")
                    ),
                    "mineru_remote_enabled": bool(
                        final_stage1_route_snapshot.get("mineru_remote_enabled")
                    ),
                    "mineru_attempted": bool(
                        final_stage1_route_snapshot.get("mineru_attempted")
                    ),
                    "mineru_succeeded": bool(
                        final_stage1_route_snapshot.get("mineru_succeeded")
                    ),
                    "mineru_route": str(
                        final_stage1_route_snapshot.get("mineru_route") or ""
                    ),
            ...
            if self.concept_mode and self.concept_profile and ai_result:
                self.logger.info(
                    f"正在对 '{paper.get('title', '未知标题')}' 进行概念增强分析..."
                )

                # 读取概念分析提示词模板
                try:
                    with open(
                        "prompts/prompt_concept_analysis.txt", "r", encoding="utf-8"
                    ) as f:
                        concept_prompt_template = f.read()
                    self.logger.success(
                        f"加载概念分析提示词模板: {len(concept_prompt_template)}字符"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"无法加载概念分析提示词模板，使用默认提示词: {e}"
                    )
                ...
                concept_prompt = concept_prompt_template.replace(
                    "{{CONCEPT_PROFILE}}",
                    json.dumps(self.concept_profile, ensure_ascii=False),
                ).replace(
                ...
                # 调用概念分析接口
                concept_analysis_result = get_concept_analysis(
                    concept_prompt,
                    writer_api_config,
                    logger=self.logger,
                    config=self.config,
            ...
            try:
                ...
                if False:
                    ...
                    valid_title = extracted_title and extracted_title not in [
                        "",
                        "未知",
                        "N/A",
                        "无标题",
                    ]
                    valid_year = extracted_year and extracted_year not in [
                        "",
                        "未知",
                        "N/A",
                        "未知年份",
                    ]
                    valid_journal = extracted_journal and extracted_journal not in [
                        "",
                        "未知",
                        "N/A",
                        "未知期刊",
                    ...
                    if extracted_authors:
                        if isinstance(extracted_authors, list):
                            ...
                            if authors_str and authors_str not in ["", "未知", "N/A"]:
                                ...
                                authors_list = re.split(
                                    r"[,，、;；和and]\s*", authors_str
                                )
                                authors_list = [
                                    author.strip()
                    ...
                    if updated_fields:
                        self.logger.info(
                            f"✅ 元数据回填成功，更新字段: {', '.join(updated_fields)}"
                        )
                    else:
    ...
    def generate_failure_report(self) -> bool:
        ...
        try:
            ...
            failure_report_file = self._get_report_file_path(
                "_failed_papers_report.txt"
            )
    ...
    def generate_retry_zotero_report(self) -> bool:
        ...
        try:
            ...
            retry_report_file = self._get_report_file_path(
                "_zotero_report_for_retry.txt"
            )
    ...
    def _paper_reuse_entry(
        self, paper: Mapping[str, Any], *, paper_key: str
    ) -> Dict[str, Any]:
        return {
            "paper_key": paper_key,
            "title": str(paper.get("title") or ""),
    ...
    def _build_stage1_reuse_report(
        self, sources: List[SummarySource], rejected_candidates: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        source_items = [
            {
                "path": source.path,
    ...
    def _apply_stage1_cross_run_reuse(self) -> bool:
        ...
        self.logger.info(
            f"[stage1 reuse] Found {len(sources)} candidate summary sources"
        )
        ...
        try:
            ...
            self.logger.error(
                f"[stage1 reuse] Failed to load an explicit reuse source: {exc}"
            )
            return False
        ...
        for paper in self.papers:
            ...
            if (
                paper_key in self._checkpoint_processed_papers
                or paper_key in self._checkpoint_failed_papers
            ):
            ...
            if match is None:
                report["not_reused"].append(
                    {**paper_entry, "reason": "no_matching_summary"}
                )
            ...
            for index, existing_summary in enumerate(self.summaries):
                ...
                if (
                    LiteratureReviewGenerator.get_paper_key(existing_paper_info)
                    == paper_key
                ):
        ...
        if reused_any:
            ...
            self.logger.info(
                f"[stage1 reuse] Reused {len(report['reused_papers'])} papers via global summary catalog"
            )
        else:
            self.logger.info(
                "[stage1 reuse] No reusable summaries matched the current paper set"
            )
    ...
    def _persist_stage1_reuse_report(self) -> bool:
        ...
        for summary in self.summaries:
            ...
            paper_key = LiteratureReviewGenerator.get_paper_key(dict(paper_info))
            if (
                paper_key not in current_paper_keys
                or paper_key in self._stage1_reused_paper_keys
            ):
                continue
        ...
        self._register_workspace_artifact(
            ...
            depends_on=[
                ArtifactDependencyRef(
                    artifact_type="summary_file", path=self.summary_file or ""
                ),
            ]
            if self.summary_file
            else [],
    ...
    def process_all_papers(self) -> bool:
        ...
        try:
            ...
            # 确定最大工作线程数
            performance_config: Dict[str, str] = (
                self.config.get("Performance", {}) if self.config else {}
            )
            ...
            if self.mode == "zotero":
                ...
                if library_path:
                    ...
                    file_index = create_file_index(library_path)
                    self.logger.success(
                        f"文件索引创建完成，包含 {len(file_index)} 个文件"
                    )
            ...
            self.logger.info(
                f"需要处理: {len(papers_to_process)}篇论文，跳过: {skipped_count}篇论文"
            )
            ...
            # 创建进度条
            progress_bar = tqdm(
                total=len(papers_to_process), desc="[阶段一] 正在分析文献"
            )
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                future_to_paper: Dict[
                    concurrent.futures.Future["ProcessingResult | None"],
                    Tuple[int, "PaperInfo"],
                ] = {
                    executor.submit(
                        self.process_paper,
                ...
                for future in concurrent.futures.as_completed(future_to_paper):
                    ...
                    try:
                        ...
                        if result and result.get("status") == "success":
                            ...
                            completed_label = self._paper_progress_label(
                                result.get("paper_info", paper)
                            )
                            self._emit_stage1_progress(
                                total=tracked_total,
                            ...
                            progress_bar.set_postfix_str(
                                f"成功: {self.processed_count.value}, 失败: {self.failed_count.value}"
                            )
                        else:
                            # 处理失败
                            failure_reason = (
                                result.get("failure_reason") or "未知错误"
                                if result
                                else "处理返回空结果"
                            )
                            ...
                            failed_paper = cast(
                                PaperInfo,
                                result.get("paper_info", paper) if result else paper,
                            )
                            ...
                            pdf_match_payload = (
                                result.get("pdf_match") if result else None
                            )
                            if isinstance(pdf_match_payload, Mapping):
                                failed_entry["pdf_match"] = dict(pdf_match_payload)
                            identity_payload = (
                                result.get("source_identity") if result else None
                            )
                            if (
                                ...
                                failed_entry["identity_verdict"] = str(
                                    result.get("identity_verdict") or ""
                                )
                                failed_entry["artifact_status"] = str(
                                    result.get("artifact_status") or ""
                                )
                        ...
                        if result and result.get("status") == "success":
                            ...
                            if not save_result:
                                self.logger.error(
                                    "⚠️ 警告: 数据保存失败，请检查磁盘空间和权限"
                                )
                        else:
                            # 失败的情况下定期保存（每3个失败保存一次）
                            if (
                                self.processed_count.get_value()
                                + self.failed_count.get_value()
                            ) % 3 == 0:
                                self.save_summaries()
                                self.save_checkpoint()
                        ...
                        self.logger.error(
                            f"失败: {self.processed_count.value}成功, {self.failed_count.value}失败 - {failure_reason}"
                        )
            ...
            self._emit_stage1_progress(
                ...
                success_count=run_success_count,
                failure_count=run_failure_count,
                item_label=self._paper_progress_label(papers_to_process[-1][1])
            ...
            if self.failed_papers:
                self.logger.warning(
                    f"有{len(self.failed_papers)}篇论文处理失败，启动自动重试循环..."
                )

                # 读取重试配置
                retry_config: Dict[str, str] = (
                    self.config.get("Retry_Settings", {}) if self.config else {}
                )
                ...
                self.logger.info(
                    f"🔄 重试配置: 最大重试轮数={max_retry_rounds}, 基础间隔={base_retry_delay}秒, 最大间隔={max_retry_delay}秒"
                )
                ...
                for failed_item in self.failed_papers:
                    ...
                    # 检查失败原因是否包含可重试关键词
                    is_retriable = any(
                        keyword in failure_reason for keyword in retriable_keywords
                    )
                ...
                for retry_round in range(1, max_retry_rounds + 1):
                    ...
                    if retry_round > 1:
                        ...
                        self.logger.info(
                            f"第 {retry_round - 1} 轮重试失败，等待 {retry_delay} 秒让API限制恢复..."
                        )
                        self.logger.info(
                            f"⏰ 间隔计算: {retry_round} × {base_retry_delay} = {calculated_delay}秒，已限制上限为 {max_retry_delay}秒"
                        )
                        self.logger.info(
                            "⏳ 等待中... 这有助于避免API频率限制，提高重试成功率"
                        )
                    ...
                    self.logger.info(
                        f"正在对 {len(retriable_failures)} 篇失败文献进行第 {retry_round} 轮自动重试..."
                    )
                    ...
                    for failed_item in retriable_failures:
                        ...
                        for i, original_paper in enumerate(self.papers):
                            if LiteratureReviewGenerator.get_paper_key(
                                original_paper
                            ) == LiteratureReviewGenerator.get_paper_key(paper_info):
                                # 计算论文key
                                paper_key = LiteratureReviewGenerator.get_paper_key(
                                    original_paper
                                )
                                ...
                                if paper_key in self._checkpoint_failed_papers:
                                    self._checkpoint_failed_papers.discard(paper_key)
                                    self.logger.info(
                                        f"已从失败集合中移除论文以便重试: {original_paper.get('title', '未知标题')}"
                                    )
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        ...
                        retry_progress_bar = tqdm(
                            ...
                            total=len(retry_papers),
                            desc=f"[重试第{retry_round}轮] 正在重试文献",
                        )
                            ...
                            try:
                                ...
                                if result and result.get("status") == "success":
                                    ...
                                    with self.save_lock:
                                        ...
                                        self._checkpoint_processed_papers.add(paper_key)
                                        # 从失败集合中移除，保持状态一致性
                                        self._checkpoint_failed_papers.discard(
                                            paper_key
                                        )
                                        # 从失败列表中移除
                                        self.failed_papers = [
                                            fp
                                            for fp in self.failed_papers  # type: ignore
                                            if LiteratureReviewGenerator.get_paper_key(
                                                fp.get("paper_info", {})
                                            )
                                            != paper_key
                                    ...
                                    retry_progress_bar.set_postfix_str(
                                        f"成功: {self.processed_count.value}, 失败: {self.failed_count.value}"
                                    )
                                else:
                                    # 重试仍然失败
                                    failure_reason = (
                                        result.get("failure_reason", "重试失败")
                                        if result
                                        else "重试返回空结果"
                                    )
                                    ...
                                    if isinstance(retry_identity, Mapping):
                                        retry_failure["source_identity"] = dict(
                                            retry_identity
                                        )
                                        retry_failure["identity_verdict"] = str(
                                            result.get("identity_verdict") or ""
                                        )
                                        retry_failure["artifact_status"] = str(
                                            result.get("artifact_status") or ""
                                        )
                    ...
                    if current_round_failures:
                        self.logger.warning(
                            f"第 {retry_round} 轮重试后，仍有 {len(current_round_failures)} 篇论文失败"
                        )
                    else:
                        self.logger.success(
                            f"第 {retry_round} 轮重试成功，所有论文处理完成！"
                        )
                        break
                ...
                self.logger.info(
                    f"📊 使用配置: {max_retry_rounds}轮重试，基础间隔{base_retry_delay}秒，上限{max_retry_delay}秒"
                )
            ...
            if self.failed_papers:
                self.logger.warning(
                    f"有{len(self.failed_papers)}篇论文处理失败，将生成失败报告"
                )
            ...
            self.logger.error("\n\n用户中断处理")
            self.logger.info(
                f"已处理: {self.processed_count.value}篇文献，失败: {self.failed_count.value}篇"
            )
            self.save_summaries()
            self.save_checkpoint()
            return False
    ...
    def run_stage_one(self, override_zotero_report_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if override_zotero_report_path:
                self.logger.info(
                    f"[重跑模式] 已将文献来源强制指定为 -> {override_zotero_report_path}"
                )
            ...
            if not self.load_existing_summaries():
                self.logger.error(
                    "Existing summary state is unsafe to reuse; aborting Stage 1"
                )
                return False
            ...
            if checkpoint_loaded:
                ...
                self.logger.info(
                    "[断点恢复] 成功加载旧 checkpoint，将从上次中断处继续处理"
                )
                self.logger.info("[断点续传] 已加载处理进度，将跳过已处理的论文")
            elif progress_snapshot_path and self._restore_stage1_progress_from_snapshot(
                progress_snapshot_path
            ):
                self._merge_stage1_progress_from_loaded_summaries()
                self.logger.info(
                    "[进度恢复] 找到新的 progress snapshot，将从上次中断处继续处理"
                )
                self.logger.info("[断点续传] 已加载处理进度，将跳过已处理的论文")
            elif self._rebuild_stage1_progress_from_loaded_summaries():
                self.logger.info(
                    "[进度恢复] 未找到有效 checkpoint，但已根据现有摘要恢复处理进度"
                )
                self.logger.info(
                    "[断点续传] 已根据摘要重建跳过集合，将跳过已处理的论文"
                )
            else:
                self.logger.info(
                    "[全新开始] 未找到有效断点或 progress snapshot，将开始全新处理"
                )
    ...
    def extract_section_title_from_outline(
        self, outline_content: str, section_number: int
    ) -> Optional[str]:
        """从大纲内容中提取指定章节的标题"""
        try:
    ...
    def create_literature_review_section(
        self, section_number: int, section_title: str, outline_content: str
    ) -> bool:
        """创建文献综述的指定章节内容"""
        try:
            section_content = self.generate_review_section_content(
                section_title, outline_content
            )
            ...
            # 将章节内容追加到Word文档
            success = self.append_section_to_word_document(
                section_number, section_title, section_text, word_file
            )

            if success:
                self._clear_failed_review_section(section_number)
                self.logger.success(
                    f"第{section_number}章已追加到文献综述: {word_file}"
                )
                return True
            else:
                return False
    ...
    def generate_review_section_content(
        self, section_title: str, outline_content: str
    ) -> Optional[str]:
        ...
        try:
            ...
            optimized_context: str = optimize_context_for_synthesis(
                self.summaries, outline_content, max_tokens=950000
            )
            ...
            # 直接使用优化的prompt文件
            with open(
                "prompts/optimized_prompt_synthesize_section.txt", "r", encoding="utf-8"
            ) as f:
                prompt_template = f.read()

            # 替换占位符
            section_prompt: str = prompt_template.replace(
                "{{SUMMARIES_JSON_ARRAY}}", optimized_context
            )
            section_prompt = section_prompt.replace("{{SECTION_TITLE}}", section_title)
            section_prompt = section_prompt.replace(
                "{{REVIEW_OUTLINE}}", outline_content
            )
            ...
            while continuation_attempts <= max_continuation_attempts:
                if continuation_attempts == 0:
                    ...
                    result = self._call_section_api_optimized(
                        section_prompt, writer_api_config, is_continuation=False
                    )
                else:
                    # 续写调用，使用续写提示词
                    self.logger.info(
                        f"[章节续写] 第{continuation_attempts}次续写: {section_title}"
                    )
                    result = self._call_section_api_optimized(
                        section_prompt,
                        writer_api_config,
                ...
                if not section_content or len(section_content.strip()) < 100:
                    self.logger.warning(
                        f"[章节生成] 返回内容过短({len(section_content)}字符)，重试..."
                    )
                ...
                self.logger.success(
                    f"[章节生成] 本次生成 {len(section_content)} 字符，累计 {len(partial_section_content)} 字符"
                )
                ...
                if finish_reason == "stop":
                    ...
                    if continuation_attempts > max_continuation_attempts:
                        self.logger.warning(
                            f"[章节生成] 达到最大续写次数({max_continuation_attempts})，返回部分生成的内容"
                        )
                        return partial_section_content
                else:
                    self.logger.warning(
                        f"[章节生成] 未知的finish_reason: {finish_reason}，假设完成"
                    )
                    return partial_section_content
    ...
    def _call_section_api(
        self,
        section_title: str,
        summaries_string: str,
        outline_string: str,
        ...
        try:
            ...
            try:
                with open(
                    "prompts/prompt_system_section.txt", "r", encoding="utf-8"
                ) as f:
...
7. 不要使用传统的 (作者, 年份)、作者/年份、标题、拼音/罗马化中文或 [[cite:paper_key]] 引用"""
            ...
            if is_continuation:
                try:
                    with open(
                        "prompts/prompt_continue_section.txt", "r", encoding="utf-8"
                    ) as f:
                        section_prompt_template = f.read()
                    self.logger.success(
                        f"加载章节续写提示词模板: {len(section_prompt_template)}字符"
                    )
                except Exception as e:
                    self.logger.warning(
                        f"无法加载章节续写提示词模板，使用默认提示词: {e}"
                    )
                    section_prompt_template = "【角色】你是一位正在撰写综述特定章节的学者，刚才思路被打断了。\n【任务】请你继续完成一份未写完的章节正文。\n\n【全部论文分析数据】\n{{SUMMARIES_JSON_ARRAY}}\n\n【综述完整大纲】\n{{REVIEW_OUTLINE}}\n\n【当前需要撰写的章节标题】\n{{SECTION_TITLE}}\n\n【已完成的章节草稿】\n{{PARTIAL_SECTION_CONTENT}}"

                final_prompt = section_prompt_template.replace(
                    "{{SUMMARIES_JSON_ARRAY}}", summaries_string
                )
                final_prompt = final_prompt.replace(
                    "{{REVIEW_OUTLINE}}", outline_string
                )
                final_prompt = final_prompt.replace("{{SECTION_TITLE}}", section_title)
                final_prompt = final_prompt.replace(
                    "{{PARTIAL_SECTION_CONTENT}}", partial_content
                )
            else:
                try:
                    with open(
                        "prompts/prompt_synthesize_section.txt", "r", encoding="utf-8"
                    ) as f:
                        section_prompt_template = f.read()
                    self.logger.success(
                        f"加载章节提示词模板: {len(section_prompt_template)}字符"
                    )
                except Exception as e:
            ...
            result = self._call_writer_api_text_detailed(
                prompt=final_prompt,
                api_config=writer_api_config,
                system_prompt=system_prompt,
                max_tokens=6000,
                temperature=0.7,
                writer_stage="review_section_legacy",
            )
    ...
    def _call_section_api_optimized(
        self,
        section_prompt: str,
        writer_api_config: "APIConfig",
        is_continuation: bool = False,
...
继续内容："""
    ...
    def append_section_to_word_document(
        self, section_number: int, section_title: str, section_text: str, word_file: str
    ) -> bool:
        """将章节内容追加到Word文档（带样式配置）"""
        return append_section_to_word_document(
            self, section_number, section_title, section_text, word_file
        )
    ...
    def generate_full_review_from_outline(self) -> bool:
        ...
        try:
            ...
            if outline_artifact is None:
                self.logger.error(
                    f"大纲文件不存在: {outline_file}，请先运行 --generate-outline 生成大纲"
                )
                return False
            ...
            section_matches = re.findall(
                r"^##\s*(\d+)\.\s*(.*)", outline_content, re.MULTILINE
            )
            ...
            for i in range(1, len(section_numbers)):
                if section_numbers[i] != section_numbers[i - 1] + 1:
                    self.logger.error(
                        f"大纲章节编号不连续：发现第{section_numbers[i - 1]}章后直接是第{section_numbers[i]}章"
                    )
                    self.logger.error(
                        "请检查大纲文件，确保章节编号连续（如1, 2, 3...）"
                    )
                    return False
            ...
            if os.path.exists(review_checkpoint_file):
                try:
                    with open(review_checkpoint_file, "r", encoding="utf-8") as f:
                        ...
                        last_completed_section = checkpoint.get(
                            "last_completed_section", 0
                        )
            ...
            if last_completed_section == 0 and os.path.exists(word_file):
                self.logger.info(
                    f"检测到已存在的旧综述文件，将创建全新版本: {word_file}"
                )
                try:
                    os.remove(word_file)
                except Exception as e:
            ...
            if os.path.exists(word_file) and last_completed_section > 0:
                ...
                for run in title.runs:
                    ...
                    run.font.size = Pt(
                        font_size_heading1 + 2
                    )  # 主标题稍大  # type: ignore

                # 添加生成时间
                date_para = doc.add_paragraph(
                    f"生成时间: {_format_chinese_datetime(datetime.now())}"
                )
            ...
            for i, (section_num, section_title) in progress_bar:
                ...
                if "参考文献" in section_title or "附录" in section_title:
                    self.logger.info(
                        f"[跳过] 第{section_num}章 '{section_title}' 将在最后由程序自动生成。"
                    )
                    continue

                # 更新进度条的当前章节信息
                progress_bar.set_postfix_str(
                    f"当前章节: {section_num}. {section_title[:30]}..."
                )
                ...
                if not section_citation_validation.get("valid"):
                    ...
                    if retry_result and retry_validation.get("valid"):
                        ...
                        self.logger.success(
                            f"TOKEN_RETRY_SUCCESS section={section_num}"
                        )
                    else:
                        self.logger.error(
                        ...
                        failed_sections.append(
                            {
                                ...
                                or section_citation_validation,
            ...
            if not failed_sections:
                if os.path.exists(review_checkpoint_file):
                    ...
                    if not keep_checkpoints:
                        ...
                        self.logger.info(
                            "Removed review checkpoint after section generation completed."
                        )
                    else:
                        self.logger.info(
                            "Keeping review checkpoint because configuration requires it."
                        )
            else:
                self.logger.info(
            ...
            if failed_sections:
                self.logger.warning(
                    "Skipping review_draft_v1 registration because one or more review sections failed"
                )
                return False
            else:
                review_sections = [
                ...
                if not citation_manifest:
                    self.logger.error(
                        "Canonical citation_manifest_v3 was not persisted."
                    )
                    return False
                ...
                if final_docx_scan_path:
                    self.logger.info(
                        f"Final DOCX citation scan saved: {final_docx_scan_path}"
                    )
                self.logger.info(
            ...
            try:
                if self._stage2_validation_enabled():
                    ...
                    self.logger.info(
                        "第二阶段验证未在配置中启用。如需运行验证，请使用: --validate-review"
                    )
            except Exception as e:
                self.logger.error(f"第二阶段验证运行时出错: {e}")
                self.logger.info(
                    "您可以手动运行验证命令: python main.py --validate-review"
                )
    ...
    def generate_apa_references(self) -> List[str]:
        ...
        citation_manifest = self._load_citation_manifest()
        return generate_apa_references_from_manifest(
            citation_manifest, self, allow_compat_fallback=False
        )
    ...
    def _create_literature_review_outline_v2(self) -> bool:
        ...
        try:
            ...
            pipeline = V2Pipeline(
                ...
                model_dependencies=(
                    [profile_dependency] if profile_dependency is not None else []
                ),
                prompt_context=self._resolve_free_mode_context(),
                logger=self.logger,
            )
    ...
    def create_literature_review_outline(self) -> bool:
        ...
        try:
            ...
            if self.mode == "direct":
                self.logger.info(
                    "大纲已生成。请检查并修改。然后，运行以下命令生成完整综述："
                )
                self.logger.info(
                    f'命令: python main.py --pdf-folder "{self.pdf_folder}" --generate-review'
                )
            else:
                self.logger.info(
                    "大纲已生成。请检查并修改。然后，运行以下命令生成完整综述："
                ...
                self.logger.info(
                    f'命令: python main.py --project-name "{self.project_name}" --generate-review'
                )
            return True
    ...
    def generate_review_outline(self, review_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            try:
                with open(
                    "prompts/prompt_synthesize_outline.txt", "r", encoding="utf-8"
                ) as f:
                    outline_prompt_template = f.read()
                self.logger.success(
                    f"加载大纲提示词模板: {len(outline_prompt_template)}字符"
                )
            except Exception as e:
                self.logger.warning(f"无法加载大纲提示词模板，使用默认提示词: {e}")
                try:
                    with open(
                        "prompts/prompt_default_outline.txt", "r", encoding="utf-8"
                    ) as f:
                        outline_prompt_template = f.read()
                    self.logger.success(
                        f"加载默认大纲提示词模板: {len(outline_prompt_template)}字符"
                    )
            ...
            try:
                with open(
                    "prompts/prompt_continue_outline.txt", "r", encoding="utf-8"
                ) as f:
                    continue_prompt_template = f.read()
                self.logger.success(
                    f"加载续写提示词模板: {len(continue_prompt_template)}字符"
                )
            except Exception as e:
                self.logger.warning(f"无法加载续写提示词模板，使用默认提示词: {e}")
                try:
                    with open(
                        "prompts/prompt_default_continue_outline.txt",
                        "r",
                        encoding="utf-8",
                    ) as f:
                        continue_prompt_template = f.read()
                    self.logger.success(
                        f"加载默认续写提示词模板: {len(continue_prompt_template)}字符"
                    )
                except Exception as e2:
            ...
            max_tokens_for_optimization = (
                950000  # 优化时最大token数（仅在最极端情况下触发截断）
            )

            self.logger.info(
                f"上下文token数({estimated_tokens})，使用高密度压缩格式..."
            )
            optimized_context = optimize_context_for_outline(
                self.summaries, max_tokens=max_tokens_for_optimization
            )
            self.logger.success(
                f"优化后的上下文长度: {len(optimized_context)}字符 (原长度: {len(summaries_string)}字符)"
            )
            self.logger.info(
                f"压缩率: {len(optimized_context) / len(summaries_string):.1%}"
            )
            ...
            while continuation_attempts <= max_continuation_attempts:
                try:
                    ...
                    if continuation_attempts == 0:
                        # 首次调用：使用原始大纲提示词
                        final_prompt = outline_prompt_template.replace(
                            "{{SUMMARIES_JSON_ARRAY}}", summaries_string
                        )
                        final_prompt = self._inject_free_mode_context(final_prompt)
                        self.logger.info(
                            f"首次大纲生成，提示词长度: {len(final_prompt)}字符"
                        )
                    else:
                        # 续写调用：使用续写提示词
                        final_prompt = continue_prompt_template.replace(
                            "{{SUMMARIES_JSON_ARRAY}}", summaries_string
                        )
                        final_prompt = final_prompt.replace(
                        ...
                        final_prompt = self._inject_free_mode_context(final_prompt)
                        self.logger.info(
                            f"续写大纲生成(第{continuation_attempts}次)，提示词长度: {len(final_prompt)}字符"
                        )  # type: ignore
                    ...
                    try:
                        with open(
                            "prompts/prompt_system_outline.txt", "r", encoding="utf-8"
                        ) as f:
                            system_prompt = f.read()
                        self.logger.success(
                            f"加载大纲系统提示词模板: {len(system_prompt)}字符"
                        )
                    except Exception as e:
                        self.logger.warning(
                            f"无法加载大纲系统提示词模板，使用默认提示词: {e}"
                        )
...
5. 不要包含任何正文内容，只输出大纲"""
                    ...
                    if outline_content and len(outline_content) > 100:  # type: ignore
                        ...
                        self.logger.success(
                            f"大纲片段生成成功，当前总长度: {len(partial_outline)}字符"
                        )  # type: ignore
                        ...
                        if finish_reason == "stop":  # type: ignore
                            ...
                            self.logger.warning(
                                f"未知的完成原因: {finish_reason}，尝试续写..."
                            )
                            continuation_attempts += 1
                            continue
                    else:
                        self.logger.warning(
                            f"大纲内容过短({len(outline_content) if outline_content else 0}字符)，重试..."
                        )  # type: ignore
                        continuation_attempts += 1
                        continue
            ...
            if continuation_attempts > max_continuation_attempts:
                self.logger.error(
                    f"[ERROR] 大纲生成续写次数过多({continuation_attempts}次)，或已陷入死循环。请检查输入数据或Prompt。"
                )
    ...
    def prepare_review_data(self) -> Dict[str, Any]:
        review_data: Dict[str, Any] = {  # type: ignore
            ...
            "successful_papers": len(
                [s for s in self.summaries if s.get("status") == "success"]
            ),
            "failed_papers": len(
                [s for s in self.summaries if s.get("status") != "success"]
            ),
        ...
        for summary in self.summaries:
            ...
            paper_info = summary.get("paper_info", {})
            ai_summary: Union[AISummary, Dict[str, Any], None] = summary.get(
                "ai_summary", {}
            )
            ...
            if methodology:
                review_data["methodologies"][methodology] = (
                    review_data["methodologies"].get(methodology, 0) + 1
                )  # type: ignore
    ...
    def generate_review_content(self, review_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                self.logger.success(
                    f"加载综述提示词模板: {len(synthesize_prompt_template)}字符"
                )
            except Exception as e:
                self.logger.warning(f"无法加载综述提示词模板，使用默认提示词: {e}")
                try:
                    with open(
                        "prompts/prompt_default_synthesize.txt", "r", encoding="utf-8"
                    ) as f:
                        synthesize_prompt_template = f.read()
                    self.logger.success(
                        f"加载默认综述提示词模板: {len(synthesize_prompt_template)}字符"
                    )
                except Exception as e2:
            ...
            # 将完整的JSON字符串注入到模板中
            final_prompt = synthesize_prompt_template.replace(
                "{{SUMMARIES_JSON_ARRAY}}", summaries_string
            )
            final_prompt = self._inject_free_mode_context(final_prompt)
            ...
            try:
                with open(
                    "prompts/prompt_system_synthesize.txt", "r", encoding="utf-8"
                ) as f:
                    system_prompt = f.read()
                self.logger.success(
                    f"Loaded review system prompt template: {len(system_prompt)} characters"
                )
            except Exception as e:
                self.logger.warning(
                    f"Unable to load review system prompt template, using default prompt: {e}"
                )
...
5. Target 3000-5000 Chinese characters."""
            ...
            api_params = (
                (self.config or {}).get("API_Parameters", {}) if self.config else {}
            )
            try:
            ...
            self.logger.error(
                f"Review generation failed: {result.get('message', 'empty response')}"
            )
            return None
    ...
    def format_review_content(
        review_content: Dict[str, Any], review_data: Dict[str, Any]
    ) -> str:
        ...
        try:
            ...
            def process_seed_paper(pdf_path: str) -> Optional[Dict[str, Any]]:  # type: ignore
                ...
                try:
                    ...
                    # 准备阶段一输入
                    pdf_text, _preprocess_metadata = self._prepare_stage1_input(
                        pdf_path
                    )
                    if not pdf_text or len(pdf_text.strip()) < 500:  # type: ignore
                        self.logger.warning(
                            f"种子论文阶段一输入准备失败: {os.path.basename(pdf_path)}"
                        )  # type: ignore
                        return None
                    ...
                    # 获取API配置
                    reader_api_config: APIConfig = get_reader_api_config(self.config)
                    backup_api_config: APIConfig = get_backup_reader_api_config(
                        self.config
                    )
                    ...
                    try:
                        ...
                        # 替换占位符
                        prompt_template = self._inject_free_mode_context(
                            prompt_template
                        )
                        analysis_prompt = prompt_template.replace(
                            "{{PAPER_FULL_TEXT}}", pdf_text
                        )  # type: ignore

                    except Exception as e:
                        self.logger.warning(
                            f"无法加载分析提示词模板，使用简化提示词: {e}"
                        )
                        # 简化提示词
                        analysis_prompt = (
                            f"请分析以下论文内容，生成结构化摘要：\n\n{pdf_text}"
                    ...
                    if ai_result:
                        ...
                        self.logger.success(
                            f"种子论文分析成功: {os.path.basename(pdf_path)}"
                        )
                        return {"paper_info": paper_info, "ai_summary": ai_result}
                    else:
                        self.logger.warning(
                            f"种子论文分析失败: {os.path.basename(pdf_path)}"
                        )
                        return None

                except Exception as e:
                    self.logger.error(
                        f"处理种子论文时出错 {os.path.basename(pdf_path)}: {e}"
                    )
                    return None
            ...
            concept_profile: Dict[str, Any] = self._generate_concept_profile(
                concept_name, concept_papers
            )  # type: ignore
    ...
    def _generate_concept_profile(
        self, concept_name: str, concept_papers: list[Dict[str, Any]]
        ...
        try:
            ...
            try:
                with open(
                    "prompts/prompt_prime_concept.txt", "r", encoding="utf-8"
                ) as f:
                    prompt_template = f.read()
                self.logger.success(
                    f"加载概念分析提示词模板: {len(prompt_template)}字符"
                )
            except Exception as e:
            ...
            ).replace("{{SEED_PAPERS}}", papers_json)
    ...
    try:
        ...
        if project_name:
            ...
            if len(project_name) > 100 or "\\" in project_name or "/" in project_name:
                ...
                logging.error(
                    '📝 示例：--project-name "案例分析" 而非 --project-name "C:\\Users\\123\\Desktop\\我的项目"'
                )
            ...
            if len(project_name) > 50:
                logging.warning(
                    f"Project name is quite long ({len(project_name)} chars); a shorter name is recommended."
                )
        ...
        result = JobRunner().run(
            request, cancel_token=getattr(args, "_cancel_token", None)
        )
        ...
        if isinstance(
            ...
            logging.error(
                "请在网络恢复后，重新运行您刚才使用的命令，程序将从中断的地方继续。"
            )
        else:
...
def parse_failure_report(
    failure_report_file: str, pdf_folder: Optional[str] = None
) -> List[PaperInfo]:  # type: ignore
    """从失败报告文件中解析失败的论文信息"""
    try:
        ...
        for title in title_matches:
            ...
            if title:
                    ...
                    for pdf_file in all_pdfs:
                        ...
                        if (
                            ...
                            if keyword_overlap > 0:
                                logging.info(
                                    f"匹配到 {keyword_overlap} 个关键词: {matched_words}"
                                )
                        ...
                        # 方法4：直接包含检查（如果PDF文件名包含标题的主要部分）
                            ...
                            if len(clean_title) > 10 and clean_title in clean_filename:
                                pdf_path = pdf_file
                                logging.info(
                                    f"基于整体字符串匹配找到PDF文件: {pdf_file}"
                                )
                                break
                            elif (
                                len(clean_filename) > 10
                                and clean_filename in clean_title
                            ):
                                pdf_path = pdf_file
                ...
                if pdf_path and os.path.exists(pdf_path):
                    ...
                    logging.info(
                        f"PDF文件夹是否存在: {os.path.exists(pdf_folder) if pdf_folder else 'None'}"
                    )
...
def handle_retry_failed(args: argparse.Namespace):  # type: ignore
    ...
    if not args.project_name and not args.pdf_folder:
        logging.error(
            "使用--retry-failed命令时必须提供--project-name或--pdf-folder参数中的一个"
        )
        sys.exit(1)
    ...
    if args.project_name:
        if (
            len(args.project_name) > 100
            or "\\" in args.project_name
            or "/" in args.project_name
        ):
            logging.error("❌ --project-name 参数错误")
            logging.error("💡 请不要使用完整路径，应该使用简洁的项目名称")
    ...
    if generator.mode == "zotero":
        retry_report_file = generator._get_report_file_path(
            "_zotero_report_for_retry.txt"
        )
        if not os.path.exists(retry_report_file):  # type: ignore:
            generator.logger.error(
                f"Zotero模式重试失败：未找到重跑报告文件 '{retry_report_file}'"
            )
            sys.exit(1)
        papers_to_retry = parse_zotero_report(retry_report_file)  # type: ignore
    else:  # direct mode
        generator.logger.info(
            "直接PDF模式：正在从摘要文件和失败报告中识别失败的论文..."
        )

        # 首先尝试从summaries.json中查找失败的论文
        failed_summaries = [
            s for s in generator.summaries if s.get("status") == "failed"
        ]  # type: ignore
        papers_to_retry = [
            s.get("paper_info") for s in failed_summaries if s.get("paper_info")
        ]  # type: ignore

        # 如果没有在summaries.json中找到失败的论文，尝试从失败报告文件中读取
        if not papers_to_retry:
            generator.logger.info(
                "在summaries.json中未找到失败的论文，正在检查失败报告..."
            )
            failure_report_file = generator._get_report_file_path(
                "_failed_papers_report.txt"
            )
            ...
            if os.path.exists(failure_report_file):
                ...
                try:
                    # 解析失败报告文件，传入PDF文件夹路径
                    failed_papers_from_report = parse_failure_report(
                        failure_report_file, generator.pdf_folder
                    )
                    if failed_papers_from_report:
                        papers_to_retry = failed_papers_from_report
                        generator.logger.info(
                            f"从失败报告中提取到 {len(papers_to_retry)} 篇需要重试的论文"
                        )
                    else:
    ...
    file_index_path: str = (
        generator.config.get("Paths", {}).get("library_path", "")
        if generator.mode == "zotero" and generator.config
        else generator.pdf_folder or ""
    )  # type: ignore
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:  # type: ignore
        future_to_paper = {
            executor.submit(
                generator.process_paper, paper, i, file_index, len(papers_to_retry)
            ): paper
            for i, paper in enumerate(papers_to_retry)
        }  # type: ignore
        progress_bar = tqdm(
            concurrent.futures.as_completed(future_to_paper),
            total=len(papers_to_retry),
            desc="[重试模式] 正在处理",
        )  # type: ignore
        for future in progress_bar:
            ...
            if result and result.get("status") == "success":  # type: ignore
                ...
                if generator.mode == "direct":
                    paper_key = LiteratureReviewGenerator.get_paper_key(
                        result.get("paper_info", {})
                    )  # type: ignore
                    # 查找并更新原始条目
                    for i, summary in enumerate(generator.summaries):
                        if (
                            LiteratureReviewGenerator.get_paper_key(
                                summary.get("paper_info", {})
                            )
                            == paper_key
                        ):  # type: ignore
                ...
                # 处理失败的论文
                failed_paper: Dict[str, Any] = result or {
                    "paper_info": future_to_paper[future],
                    "failure_reason": "未知重试错误",
                }  # type: ignore
                if generator.mode == "direct":
                    paper_key = LiteratureReviewGenerator.get_paper_key(
                        failed_paper.get("paper_info", {})
                    )
                    # 查找并更新原始条目
                    for i, summary in enumerate(generator.summaries):
                        if (
    ...
    # 计算新增成功的论文数量
    success_count = len(
        [s for s in generator.summaries if s.get("status") == "success"]
    )  # type: ignore
    original_success = len(
        [
            s
            for s in generator.summaries[:original_summary_count]
    ...
    generator.logger.success(
        f"重试完成！新增成功 {newly_succeeded} 篇，仍然失败 {failed_count} 篇。"
    )  # type: ignore

    if (
        not generator.failed_papers
        and generator.mode == "zotero"
        and os.path.exists(retry_report_file)
    ):
        try:
            os.remove(retry_report_file)
            generator.logger.info(
                f"所有失败论文均已成功重试，已自动删除重跑报告文件: {retry_report_file}"
            )
        except Exception as e:
...
def handle_merge_mode(args: argparse.Namespace):  # type: ignore
    ...
    if not args.project_name and not args.pdf_folder:
        logging.error(
            "使用--merge命令时必须提供--project-name或--pdf-folder参数中的一个"
        )
        sys.exit(1)
    ...
    try:
        ...
        try:
            main_data = load_json_file_with_fallbacks(
                main_file, logger=generator.logger
            )  # type: ignore
        except Exception as exc:
        ...
        try:
            merge_data = load_json_file_with_fallbacks(
                merge_file, logger=generator.logger
            )  # type: ignore
        except Exception as exc:
        ...
        def get_paper_key(paper: "Dict[str, Any] | PaperInfo"):  # type: ignore
            ...
            return paper_info.get(
                "doi", f"{paper_info.get('title', '')}_{paper_info.get('authors', [])}"
            )  # type: ignore
...
def handle_run_all_mode(generator: "LiteratureReviewGenerator"):  # type: ignore
    ...
    if stage1_success:
        ...
        existing_v2_outline = (
            generator._load_outline_artifact()
            if generator._outline_v2_enabled()
            else None
        )
        if existing_v2_outline is not None:
            generator.logger.info(
                "检测到有效的 v2 adopted outline，将跳过大纲重建并继续生成全文"
            )
        ...
        if outline_success:
            ...
            if needs_v2_adoption and not generator.adopt_outline_v2(
                adopted_by="run_all"
            ):
...
def handle_generate_outline_mode(
    generator: "LiteratureReviewGenerator", args: argparse.Namespace
):  # type: ignore
    """处理生成大纲模式"""
    ...
    if success:
        ...
        if args.project_name:
            ...
            if args.concept:
                generator.logger.info(
                    f'命令: python main.py --project-name "{args.project_name}" --concept "{args.concept}" --generate-review'
                )
            else:
                generator.logger.info(
                    f'命令: python main.py --project-name "{args.project_name}" --generate-review'
                )
        elif args.pdf_folder:
            # 检查是否是概念模式
            if args.concept:
                generator.logger.info(
                    f'命令: python main.py --pdf-folder "{args.pdf_folder}" --concept "{args.concept}" --generate-review'
                )
            else:
                generator.logger.info(
                    f'命令: python main.py --pdf-folder "{args.pdf_folder}" --generate-review'
                )
    else:
        generator.logger.error("\n大纲生成失败！")
        sys.exit(1)
...
def handle_generate_section_mode(
    generator: "LiteratureReviewGenerator", args: argparse.Namespace
):  # type: ignore
...
def handle_stage_one_mode(
    ...
    if stage1_success:
        ...
        if args.project_name:
            ...
            if args.concept:
                generator.logger.info(
                    f'生成大纲: python main.py --project-name "{args.project_name}" --concept "{args.concept}" --generate-outline'
                )
                generator.logger.info(
                    f'一键生成综述: python main.py --project-name "{args.project_name}" --concept "{args.concept}" --run-all'
                )
            else:
                generator.logger.info(
                    f'生成大纲: python main.py --project-name "{args.project_name}" --generate-outline'
                )
                generator.logger.info(
                    f'一键生成综述: python main.py --project-name "{args.project_name}" --run-all'
                )
        elif args.pdf_folder:
            # 检查是否是概念模式
            if args.concept:
                generator.logger.info(
                    f'生成大纲: python main.py --pdf-folder "{args.pdf_folder}" --concept "{args.concept}" --generate-outline'
                )
                generator.logger.info(
                    f'一键生成综述: python main.py --pdf-folder "{args.pdf_folder}" --concept "{args.concept}" --run-all'
                )
            else:
                generator.logger.info(
                    f'生成大纲: python main.py --pdf-folder "{args.pdf_folder}" --generate-outline'
                )
                generator.logger.info(
                    f'一键生成综述: python main.py --pdf-folder "{args.pdf_folder}" --run-all'
                )
    else:
        generator.logger.error("\n阶段一执行失败！")
        sys.exit(1)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    base_group.add_argument(
        "--project-name", "-p", type=str, help="项目名称（用于创建独立的输出文件夹）"
    )
    base_group.add_argument(
        "--pdf-folder",
        "-f",
    ...
    workflow_group.add_argument(
        "--setup", action="store_true", help="运行交互式设置向导"
    )
    workflow_group.add_argument(
        "--run-all", "-a", action="store_true", help="一键运行：分析 → 大纲 → 综述"
    )
    workflow_group.add_argument(
        "--analyze-only", "-A", action="store_true", help="仅运行阶段一：文献分析"
    )
    workflow_group.add_argument(
        "--generate-outline",
        "-o",
    ...
    workflow_group.add_argument(
        "--generate-review", "-r", action="store_true", help="仅运行阶段三：生成综述"
    )
    workflow_group.add_argument(
        "--validate-review", "-v", action="store_true", help="验证生成的综述"
    )
    workflow_group.add_argument(
        "--outline-adopt", action="store_true", help="显式采纳 Week5 大纲仲裁结果"
    )
    workflow_group.add_argument(
        "--adopt-outline-v2",
        action="store_true",
    ...
    workflow_group.add_argument(
        "--cleanup", action="store_true", help="清理旧工作空间，只保留最新的"
    )

    # 高级选项
    advanced_group = parser.add_argument_group("高级选项")
    advanced_group.add_argument(
        "--prime-with-folder", type=str, help="概念预热：指定种子论文文件夹"
    )
    advanced_group.add_argument("--concept", type=str, help="概念预热：指定概念名称")
    advanced_group.add_argument(
        "--retry-failed", action="store_true", help="重试失败的论文"
    )
    advanced_group.add_argument("--generate-section", type=int, help="仅生成指定章节号")
    advanced_group.add_argument(
        "--retry-review-failed", action="store_true", help="重试失败的综述章节"
    )
    advanced_group.add_argument(
        "--free-mode-profile", type=str, help="Free mode: load a profile JSON file"
    )
    advanced_group.add_argument(
        "--free-mode-idea", type=str, help="Free mode: pass an idea as plain text"
    )
    advanced_group.add_argument(
        "--merge",
        type=str,
    ...
    zotero_group.add_argument("--zotero-report", type=str, help="Zotero报告文件路径")
    zotero_group.add_argument("--library-path", type=str, help="Zotero库路径")
...
def handle_cleanup_mode(args: argparse.Namespace):  # type: ignore
    ...
    try:
        ...
        if args.project_name:
            ...
            project_name = sanitize_path_component(
                os.path.basename(os.path.abspath(args.pdf_folder.rstrip("/\\")))
            )
        ...
        # 加载配置以获取输出路径
        temp_generator = LiteratureReviewGenerator(
            args.config, project_name, args.pdf_folder, "output/_queue/queue.json"
        )
        ...
        paths_config = (
            temp_generator.config.get("Paths", {}) if temp_generator.config else {}
        )
        ...
        def score_workspace(workspace_path: str) -> int:
            ...
            # 检查重要文件
            if os.path.exists(
                os.path.join(artifacts_dir, f"{project_name}_summaries.json")
            ):
                score += 10
            if os.path.exists(
                os.path.join(
                    artifacts_dir, f"{project_name}_literature_review_outline.md"
                )
            ):
                score += 10
            if os.path.exists(
                os.path.join(reports_dir, f"{project_name}_literature_review.docx")
            ):
                score += 10
            if os.path.exists(
                os.path.join(reports_dir, f"{project_name}_analyzed_papers.xlsx")
            ):
                score += 5
        ...
        print(
            f"找到 {len(workspaces)} 个工作空间，将保留文件最完整的 1 个，删除其余的 {len(other_workspaces)} 个:"
        )
        print(
            f"  保留: {os.path.basename(best_workspace)} (得分: {workspaces_with_scores[0][0]})"
        )
