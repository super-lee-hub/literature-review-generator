...
class JobRunner:
        ...
        if pointer_payload:
            ...
            if workspace_path and pointer_fingerprint == fingerprint_bundle and os.path.exists(workspace_path):
                return JobWorkspace.from_workspace_path(
                    ...
                    job_id=str(pointer_payload.get("job_id", "") or None),
                )
    ...
    def run(self, request: JobRunRequest) -> JobRunResult:
        ...
        compat_view = CompatConfigView.from_config(generator.config)
        output_base_dir = generator.config.get("Paths", {}).get("output_path", "./output")

        fingerprint_bundle = build_fingerprint_bundle(
            FingerprintInputs(
                config_snapshot=sanitize_config_for_fingerprint(generator.config),
        ...
        workspace = self._build_workspace(
            ...
            fingerprint_bundle=fingerprint_bundle.to_dict(),
        )
        ...
        resume_report = determine_resume_state(
            ...
            expected_fingerprint_bundle=fingerprint_bundle.to_dict(),
        )
        ...
        if request.run_all:
            legacy_main.handle_run_all_mode(generator)
        elif request.generate_outline:
            legacy_main.handle_generate_outline_mode(generator, legacy_args)
        elif request.action == "generate_section" and request.generate_section:
            legacy_main.handle_generate_section_mode(generator, legacy_args)
        elif request.generate_review:
            legacy_main.handle_generate_review_mode(generator)
        elif request.retry_review_failed:
            legacy_main.handle_retry_review_failed_mode(generator)
        elif request.validate_review:
            if generator.load_existing_summaries():
                legacy_main.validator.run_review_validation(generator)  # type: ignore[attr-defined]
            else:
                generator.logger.error("无法加载摘要文件，请先运行阶段一")
                raise SystemExit(1)
        else:
            legacy_main.handle_stage_one_mode(generator, legacy_args)
