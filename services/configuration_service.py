...
MINERU_ENV_KEYS = [
    ...
    "MINERU_MAX_REMOTE_TASKS",
    "MINERU_MAX_REMOTE_HTTP_CALLS",
    "MINERU_MAX_REMOTE_UPLOAD_BYTES",
    "MINERU_RESPONSE_MAX_BYTES",
...
class ConfigurationPersistenceError(RuntimeError):
    """Raised when the config/.env pair cannot be published atomically."""
...
def _render_env_bytes(env_values: Mapping[str, str], env_path: str) -> bytes:
    ordered_keys = [API_ENV_MAPPING[section] for section in API_ENV_MAPPING] + MINERU_ENV_KEYS
    existing = read_env_file(env_path)
    existing.update({str(key): str(value) for key, value in env_values.items()})
    lines = ["# Generated by auto-generate GUI setup", ""]
    for key in ordered_keys:
        _reject_control_text(key, label="dotenv key")
        lines.append(f"{key}={_reject_control_text(existing.get(key, ''), label=f'dotenv value {key}')}")
    ...
    if extra_keys:
        ...
        for key in extra_keys:
            ...
            lines.append(f"{key}={_reject_control_text(existing[key], label=f'dotenv value {key}')}")
    return ("\n".join(lines).rstrip() + "\n").encode("utf-8")


def _render_config_bytes(sections: Mapping[str, Mapping[str, str]]) -> bytes:
    parser = configparser.ConfigParser()
    for section_name, values in sections.items():
        _reject_control_text(str(section_name), label="config section")
        parser[section_name] = {
            _reject_control_text(str(key), label="config key"): _reject_control_text(
                str(value), label=f"config value {section_name}.{key}"
            )
...
def _stage_bytes(target: str | os.PathLike[str], payload: bytes) -> Path:
...
def _replace_staged(source: Path, target: Path) -> None:
    try:
        atomic_replace_with_retry(source, target, timeout_seconds=5.0)
    except AtomicReplaceTimeoutError as exc:
        raise ConfigurationPersistenceError(str(exc)) from exc


def _restore_bytes(target: Path, previous: bytes | None) -> None:
    if previous is None:
        target.unlink(missing_ok=True)
        return
    restore_path = _stage_bytes(target, previous)
    try:
        _replace_staged(restore_path, target)
    finally:
        restore_path.unlink(missing_ok=True)


def _publish_config_env_pair(
    config_path: str,
    config_bytes: bytes,
    env_path: str,
    env_bytes: bytes,
    ...
    try:
        _replace_staged(staged_config, config_target)
        config_replaced = True
        _replace_staged(staged_env, env_target)
        try:
        ...
        if config_replaced:
            try:
                _restore_bytes(config_target, previous_config)
            except Exception as rollback_exc:
                raise ConfigurationPersistenceError(
                    "config/.env publication failed and config rollback also failed"
                ) from rollback_exc
        raise ConfigurationPersistenceError(
            "config/.env publication failed; previous pair was preserved"
        ) from exc
    finally:
        staged_config.unlink(missing_ok=True)
...
def write_env_file(env_values: Mapping[str, str], env_path: str = ".env") -> None:
    ...
    staged = _stage_bytes(target, _render_env_bytes(env_values, str(target)))
    try:
...
def write_config_file(
    ...
    staged = _stage_bytes(target, _render_config_bytes(sections))
    try:
        _replace_staged(staged, target)
    finally:
        staged.unlink(missing_ok=True)
...
def save_config_and_env(
    ...
    schema_errors = validate_config_keys(normalized)
    if schema_errors:
        raise ValueError("configuration contains unsupported fields: " + "; ".join(schema_errors))
    _publish_config_env_pair(
        config_path,
        _render_config_bytes(normalized),
        env_path,
        _render_env_bytes(env_payload, env_path),
    )
